...
#
# growth_engine (#1368): the shared table already summed to 190, so onboarding
# growth-engine's tenant again takes it from the inert immich cap rather than
# raising max_connections — immich drops from 10 to 5 and growth_engine takes 5,
# keeping the sum at 190. Nothing but a CLI or a test run connects as
# growth_engine before it has a Deployment, so 5 is a placeholder to be re-sized
# from real boot demand once one exists.
set -euo pipefail
...
SELECT count(*) FILTER (WHERE rolconnlimit > 0) || ' ' || count(*)
  FROM pg_roles WHERE rolname IN ('authentik','forgejo','immich','garden','claws','growth_engine');
SQL
)
