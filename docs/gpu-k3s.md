...
**Depth:** **Reference**
**Read this when:** working on the GPU worker node, NVIDIA device plugin, or GPU-pinned services such as Ollama and Whisper.
**Read instead:** [nas-k3s.md](nas-k3s.md) for NFS/storage node scheduling.
...
| `ryzen` (NixOS host with NVIDIA GPU, rebuilt from Ubuntu 2026-07-30) | worker | 192.168.0.69 | Ollama, Whisper, forgejo-runner-ryzen |
...
`ryzen` also runs this repo's self-hosted GitHub Actions runner as a native (non-Kubernetes) process — separate from, and not to be confused with, the in-cluster `forgejo-runner-ryzen` pod (see [forgejo-actions.md](forgejo-actions.md)), which serves Forgejo Actions, not GitHub Actions. This is operational state declared in the sibling `nixos-config` repo, not in this repo's manifests. The GitHub Actions runner fleet is LAN-only and lives on `ryzen` and `k3s-nas` only — the `k3s` control-plane node deliberately hosts no runner (it's the least-provisioned box, and CI must not compete with Traefik/Flux for CPU). A former Hetzner VM runner (`beefy-actions`) was decommissioned 2026-08-27; since then all GitHub Actions runners for this repo have been LAN-only, never a `k3s` host.

## Access

Traefik runs on the main node and handles all ingress. k3s CNI (Flannel) routes cross-node traffic transparently. Ollama and Whisper have no external Ingress — they are accessed in-cluster via Service DNS (`http://ollama:11434`, `http://whisper:9000`).

## Services on the GPU Node

| Service | GPU | Local PVCs | Notes |
|---------|-----|------------|-------|
| Ollama | `nvidia.com/gpu: 1` | `ollama-models-pvc` (100Gi) | Models are host-local; 6 GB VRAM caps practical model size at ~7B Q4. minicpm-v (~5.5 GB) is preloaded for Garden's seed-packet OCR |
...
Models Ollama should always have on disk are declared in `OLLAMA_PRELOAD_MODELS` (space-separated) on the `model-preload` container in `apps/ollama/deployment.yaml`. Adding a model means editing that value and merging — never `kubectl exec ... ollama pull`. Pulls are idempotent and re-run on every pod start, so a rebuilt PVC re-downloads the declared models automatically. `OLLAMA_MAX_LOADED_MODELS=1` prevents two resident models on the 6 GB card. If Garden's seed-packet OCR hits CUDA OOM with Whisper warm, switch `SEED_OCR_MODEL` in `apps/garden/deployment.yaml` to `moondream` (~1.7 GB) and add it to `OLLAMA_PRELOAD_MODELS`.
...
4. In-cluster Ollama clients will fail to reach it until the pod is back
...
- **`ollama`/`whisper` pods stuck in phase `Failed` with `status.reason: UnexpectedAdmissionError` after `ryzen` reboots** (issues #713/#714, closed 2026-08-05 as one-off cleanup, **not** structurally fixed): on every `ryzen` boot, kubelet restores `nvidia.com/gpu` capacity from its device-manager checkpoint and re-admits the pods already assigned to the node *before* the nvidia-device-plugin re-registers healthy devices. Admission fails with `Pod was rejected: Allocate failed due to no healthy devices present`. The Deployment immediately schedules a working replacement — the service self-heals within seconds — but the rejected pod is left behind in phase `Failed` forever; nothing in this repo garbage-collects it. Each `ryzen` reboot adds one fresh tombstone per GPU workload, and each poll of the cluster's pod-failure monitoring re-reports every tombstone still present. **Filter on `.status.reason == "UnexpectedAdmissionError"`, never on the `kubectl get pods` STATUS column** — a tombstone can show `ContainerStatusUnknown` in that column while `.status.reason` is still `UnexpectedAdmissionError`. The `k3s-monitor` that files these issues is level-triggered on stored pod state, not on Kubernetes Events: `src/jobs/k3s-monitor.ts` polls `kubectl get pods --all-namespaces -o json` every 15 minutes and alerts on any pod with `phase == "Failed"`, with no `.status.reason` filter, no age filter and no auto-close — so a surviving tombstone re-reports on every poll forever (in #1013 the count advanced 4 → 8 in exactly one hour with no `ryzen` reboot and every Warning event long aged out of the 1h TTL). Since then this is handled automatically: the `admission-error-reaper` CronJob (`apps/admission-error-reaper/`, every 5 minutes on `k3s`) deletes exactly the pods in `default` whose `.status.reason` is `UnexpectedAdmissionError`, so a tombstone survives at most one run. Nothing else is touched — backup-Job failure tombstones have a `null` `.status.reason` and are deliberately left in place as evidence. To clear tombstones by hand between reaper runs: `kubectl delete pod -n default $(kubectl get pods -n default --field-selector status.phase=Failed -o jsonpath='{range .items[?(@.status.reason=="UnexpectedAdmissionError")]}{.metadata.name}{" "}{end}')`.
