...
# The one exception is clusters/my-cluster/claws-staging-prepull, which is
# grouped with clusters/my-cluster/claws-staging: the pre-puller only helps
# if it pulls the exact tag the StatefulSet is about to roll to (#1373).
...
OUTPUT="$(printf '%s\n' "$REFS" | awk -F'\t' '
  ...
  { n = split($2, a, ":"); if (n < 2) next          # untagged ref — skip
    ...
    dir = $1; sub(/\/[^\/]*$/, "", dir)
    # One linked group: the pre-puller must match the StatefulSet tag (#1373).
    if (dir == "clusters/my-cluster/claws-staging-prepull") dir = "clusters/my-cluster/claws-staging"
    k = dir "|" repo
