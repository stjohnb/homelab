...
**Depth:** **Reference**
**Read this when:** debugging GHCR pull auth failures or wiring a pull secret for a new service.
**Read instead:** [registry.md](registry.md) for the self-hosted registry path.
...
Images this organisation builds itself are moving off GHCR to the self-hosted zot registry at `registry.home.bstjohn.net` (see [registry.md](registry.md)); bin-scraper moved to zot on 2026-09-08, so `ghcr-pull` now serves only `claws` (claws-staging).

`ghcr-pull` also exists as a second, independently SOPS-encrypted Secret in the `claws-sessions` namespace (`clusters/my-cluster/claws-sessions/ghcr-pull-secret.enc.yaml`, #1328) so `k8s-pod` session pods can pull `ghcr.io/st-john-software/claws:<tag>` — namespace-scoped, so the `default`-namespace Secret above cannot be reused there. Both files are re-encrypted from the same PAT, so a PAT rotation must update both.

**Constraint**: do not solve pull-auth failures by making packages public. The owner explicitly wants org images to remain private while the org moves away from GitHub, so the allowed fixes are credentials, a self-hosted registry, or deployment changes that avoid the dependency entirely.
...
# Back up the keyfile somewhere safe (the operator's password manager)
...
### Non-kubelet consumer: the Forgejo runner

`apps/forgejo-runner/deployment.yaml` projects a Secret's `.dockerconfigjson` key as `/docker-config/config.json` (a volume `items:` remap — docker/cli only reads `config.json`) and sets `DOCKER_CONFIG=/docker-config` on the `runner` container, so `act_runner` can pull private job images through its dind sidecar. This consumer is invisible to `scripts/check-image-pull-secrets.sh`, which only inspects `imagePullSecrets` keys. As of the #956/#975 LAN-registry migration this projects `registry-pull` (minted by `migrations/0021-registry-credentials.sh`), not `ghcr-pull` — see [forgejo-actions.md](forgejo-actions.md#job-container-image) and [registry.md](registry.md). The pattern (non-kubelet `DOCKER_CONFIG` projection, invisible to the pull-secret CI check) is the reusable part; the specific Secret it points at has moved off GHCR.
...
3. `sops apps/ghcr-pull-secret.enc.yaml` and update the encoded auth. Repeat for `clusters/my-cluster/claws-sessions/ghcr-pull-secret.enc.yaml` — both files carry the same PAT and must be updated together.
4. Commit + PR + merge. Flux re-applies both Secrets within ~1 minute.
