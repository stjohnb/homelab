...
#
# The org's CI runs on self-hosted NixOS runners (`ryzen`, `nas`),
      ...
      devShells = forAllSystems (pkgs: {
        ...
        default = pkgs.mkShell {
          packages = with pkgs; [
            ...
            opentofu # tofu-validate job (tofu/unifi)
        ...
        # Small shell for the maintenance/notification/publish workflows that
        # only talk to remote APIs: cleanup-actions-storage.yml, renovate.yml's
        # preflight query, the bump steps of the six update-*.yml receivers
        # (bin-scraper, garden, claws-staging, forgejo, arpwatch,
        # forgejo-runner-nix), and the comment-posting step of ci.yml's
        # kustomize-diff job. `curl` and `jq` are for publish-forgejo-oidc.yml,
        # which fetches and validates Forgejo's live OIDC discovery document
        # and JWKS. Separate from `default` so those jobs don't pay for the
        # scanner toolchain closure.
        scripts = pkgs.mkShell {
          packages = with pkgs; [
            gh
            curl # publish-forgejo-oidc.yml
            jq # publish-forgejo-oidc.yml
            awscli2 # publish-oidc-discovery.yml
          ];
        };

        # Renovate CLI for .github/workflows/renovate.yml. Kept out of
        # `default` so the per-PR CI jobs don't pay for its closure.
