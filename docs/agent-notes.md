...
**Depth:** **Reference**
**Read this when:** debugging manually on the shared automation host and need a verified host/kubeconfig/CI gotcha.
**Read instead:** [OVERVIEW.md](OVERVIEW.md) for subsystem architecture and the full doc map.
...
Why this matters: both clusters expose similarly named resources and secrets, so a manual debug read against the wrong kubeconfig can look plausible while returning the wrong state. Concretely, both clusters run Authentik with a `default/authentik-secrets` Secret holding the *same* OIDC key names (e.g. per-service client secrets) with *different* values — reading one from the wrong cluster returns a plausible-looking but wrong value rather than an obvious error. Before any out-of-band inspection, verify the target with `kubectl get gitrepository -n flux-system`.
...
## The automation host's `gh` token cannot write GitHub org-level secrets

The session's default `gh` token scopes are `gist, read:org, repo, workflow` — no `admin:org`. Writing an org-level GitHub Actions secret (`PUT /orgs/{org}/actions/secrets/...`) needs `admin:org`, so that action always needs the human owner; do not attempt it as an agent and report it as a manual step instead of retrying with different scopes.
