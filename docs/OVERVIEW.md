...
| [datasette.md](datasette.md) | Browsing redacted snapshots of the Postgres databases | **Reference** |
...
| [forgejo.md](forgejo.md) | Working on the self-hosted Git forge, its storage, upgrades, the no-mirrors GitHub-copy policy, or Forgejo-side Renovate | **Reference** |
| [forgejo-backups.md](forgejo-backups.md) | Restoring or changing Forgejo's local/offsite backups | **Deep dive** |
| [forgejo-actions.md](forgejo-actions.md) | Working on the in-cluster forgejo-runner or its Docker-in-Docker sidecar and Docker access for job containers | **Reference** |
| [browser.md](browser.md) | Working on the shared browserless Chromium service, or wiring a new consumer to it | **Reference** |
...
| [unifi.md](unifi.md) | Managing the UniFi network declaratively with tofu-controller (IoT VLAN, zone firewall policies) | **Reference** |
...
│   ├── infrastructure/                #   Layer 1: cert-manager, traefik, tofu-controller (HelmRepository sources also: headlamp, nvidia-device-plugin, prometheus-community)
│   ├── config/                        #   Layer 2: certificates, RBAC (depends on infrastructure)
│   ├── tofu/                          #   Layer 2: Terraform CR for the UniFi network (see docs/unifi.md)
│   ├── kustomization.yaml             #   Root resource list (architectural safeguard)
│   ├── infrastructure-kustomization.yaml
│   ├── config-kustomization.yaml
│   ├── tofu-kustomization.yaml        #   Flux Kustomization → ./clusters/my-cluster/tofu (depends on infrastructure)
...
├── tofu/unifi/                        # OpenTofu config for the UniFi network, run in-cluster by tofu-controller
│
...
Layer 1: infrastructure  (10m interval, retryInterval: 1m, healthChecks: 2 HelmReleases, timeout: 5m)
    └── cert-manager, traefik, tofu-controller
...
Layer 2: tofu            (10m interval, retryInterval: 1m, dependsOn: infrastructure, wait: false)
    └── Terraform CR → ./tofu/unifi (UniFi network; plan-only, see docs/unifi.md)
...
The `config` layer depends on `infrastructure` (cert-manager must be running before ClusterIssuers are created). The `tofu` layer also depends on `infrastructure` and is `wait: false` — a plan-only `Terraform` CR never reports `Ready=True`, so health gating it would stall everything behind it; nothing else depends on it. See [docs/unifi.md](unifi.md). The `migrations` layer also depends on `infrastructure` and runs in parallel with `config` — it generates secrets that apps need, but does not require certificates or RBAC. The `apps` layer depends on both `config` and `migrations`, ensuring certificates exist and secrets are populated before application pods start. Because `migrations/job.yaml` is annotated `kustomize.toolkit.fluxcd.io/force: enabled` and its scripts come from a hash-suffixed `configMapGenerator`, a merged migration runs on the next reconcile with no human step — see [Secret Migration Jobs](apps-overview.md#secret-migration-jobs). The `claws-sessions` layer has no `dependsOn` and `apps` does not depend on it either, so a failure here can never block every app. It is not under `apps/` because `apps-kustomization.yaml`'s `targetNamespace: default` would rename its `Namespace` object to `default`, breaking Pod Security restrictions; it also needs SOPS decryption, which only `apps` has today, so `config`/`infrastructure` (no decryption) were rejected as homes. See [apps-overview.md](apps-overview.md).
...
| `kube-system` | nvidia-device-plugin DaemonSet (see [infrastructure-overview.md](infrastructure-overview.md#namespace-layout) for the HelmRelease namespace quirk); `local-path-config` (Flux-owned by the `claws-sessions` layer, #1382) |
...
All services use `*.home.bstjohn.net` (LAN) hostnames, and every hostname has a sibling `*.ext.bstjohn.net` on the same resource. ForwardAuth ext hosts use their own `<svc>-ext` Ingress and the `authentik-auth-ext` chain (#1111). Tailscale split DNS sends the whole `bstjohn.net` domain to the in-cluster `tailnet-dns` resolver at `100.78.7.18` (`apps/tailnet-dns/`), which answers `100.78.7.18` for `.home` and forwards the rest of the domain to public resolvers (#1264), so `.home` reaches Traefik with no subnet router; the openclaw subnet router remains the fallback for encrypted-DNS clients. `*.ext` resolves to the `k3s` node's own tailnet address and needs no subnet router either — see [Remote access over Tailscale](infrastructure-overview.md#remote-access-over-tailscale).

### SSO: Authentik ForwardAuth

Authentik (`apps/authentik/`) provides centralized SSO using two integration modes. **ForwardAuth**: five Traefik `Middleware` CRDs forming two chains — shared header stripping plus a ForwardAuth and combining chain per outpost (`authentik-auth` for `.home.`, `authentik-auth-ext` for `.ext.`) — protect 13 services (Sonarr, Radarr, Prowlarr, Bazarr, Transmission, Grafana, Prometheus, Homepage, Awtrix Kitchen, Awtrix Office, Navidrome, Music Assistant, Datasette). **Native OIDC**: 12 services (Mealie, Jellyfin, Jellyseerr, Headlamp, Proxmox, Bin Scraper, Claws, Forgejo, Seerr, Home Assistant, Container Registry, Garden) integrate via Authentik's OAuth2/OIDC provider — no ingress annotation needed. All providers, applications, users, groups, and policy bindings are configured declaratively via Authentik blueprints (`configmap-blueprints.yaml`): 25 ForwardAuth providers (13 home + 12 ext — Datasette is home-only) + 12 OIDC providers, 25 ForwardAuth applications + 12 OIDC applications (+12 provider-less ext bookmarks), 4 groups (`all-apps`, `media`, `home`, `infra`), 2 users, and 74 policy bindings. Navidrome is the one ForwardAuth service with a split router — the Subsonic API and public shares bypass ForwardAuth so mobile clients keep working; see [Navidrome: Split Router for Subsonic Clients](authentik.md#navidrome-split-router-for-subsonic-clients-962). The ForwardAuth address uses the FQDN (`authentik-server.default.svc.cluster.local`) because Traefik resolves it from the `traefik` namespace. Home Assistant's OIDC provider is wired (worker env + migration 0022) but not yet consumed HA-side — no ingress change until the `auth_oidc` component is vendored into `home-assistant-config`. Services with their own auth (Immich, Plex, Overseerr) and Gatus (kept outside ForwardAuth so monitoring remains accessible during an Authentik outage) are intentionally excluded. Authentik is a single point of failure for the ForwardAuth services, which have no login of their own; the recovery path is [Break-glass: reaching a service while Authentik is down](authentik.md#break-glass-reaching-a-service-while-authentik-is-down). See [docs/authentik.md](authentik.md).
...
| `low-priority` | 100000 | servarr services, bin-scraper, arpwatch, jellyfin, navidrome, jellyseerr, seerr, plex, pve-exporter, forgejo-runner, datasette |
...
Defined in `.github/workflows/ci.yml`. Runs on PRs targeting `main` and pushes to `main`. Self-hosted runner. fleet-infra builds no container images — every image it deploys is built in its own repository (see [registry.md](registry.md)).
...
- **`update-bin-scraper.yml`** — triggered via `workflow_dispatch` (`gh workflow run` from bin-scraper's Forgejo Actions release job on `git.home.bstjohn.net`, authorized by the org-level `FLEET_INFRA_VERSION_BUMP` PAT); validates tag format, updates the `registry.home.bstjohn.net` image reference in `apps/bin-scraper/deployment.yaml`, and opens a PR (`automation/bump-bin-scraper-<tag>` branch, `auto-bump` label) for Claws' auto-merger to merge once CI passes — no native GitHub auto-merge, since with no required status checks on `main` it races GitHub's mergeability computation unreliably. See [infrastructure-overview.md](infrastructure-overview.md) for the PAT's required scopes.
- **`update-garden.yml` / `update-forgejo.yml` / `update-claws-staging.yml` / `update-arpwatch.yml` / `update-forgejo-runner-nix.yml` / `update-cycling.yml` / `update-growth-engine.yml`** — the same `workflow_dispatch` image-bump handshake for Garden, Forgejo, `claws-staging`, arpwatch, the Forgejo Actions job image, cycling, and growth-engine. `update-forgejo.yml` validates `<version>-<sha>-rootless` and bumps **both** `apps/forgejo/deployment.yaml` and `apps/forgejo/cronjob-backup.yaml` in one PR, because the server and `forgejo dump` must run the same binary — unlike the others, its PR carries only the `dependencies` label, not `auto-bump`: `Recreate` runs a one-way SQLite migration, so the bump PR needs a manual pre-merge checklist (backup, release notes, possible `startupProbe` widening — see the Upgrade Runbook in [forgejo.md](forgejo.md)) rather than merging unattended once CI passes. `update-claws-staging.yml` differs in two ways: it validates the `vYYYY-MM-DD.N` tag format and rewrites the image reference across `apps/claws/*.yaml` (rather than a hard-coded filename, which claws#2752's StatefulSet rename would break), and it uses a single fixed branch `automation/bump-claws-staging` instead of a per-tag branch, because claws cuts a release on every push to `main` — the one open PR is force-updated to the newest tag rather than accumulating conflicting PRs. `update-arpwatch.yml`, `update-forgejo-runner-nix.yml`, `update-cycling.yml` and `update-growth-engine.yml` are all dispatched from `git.home.bstjohn.net` (arpwatch, cycling and growth-engine from their own repos, the runner image from the forgejo build repo, #1189) and carry `auto-bump`; the runner-image bump additionally needs a `kubectl rollout restart` of the three runner Deployments, which its PR body spells out. `update-growth-engine.yml`'s first run additionally flips `apps/growth-engine/deployment.yaml`'s `replicas: 0` to `1` by matching an exact marker comment — the Deployment ships scaled to zero because no image exists yet (#1381).
- **`cleanup-actions-storage.yml`** — runs weekly (Monday 04:00 UTC) and on `workflow_dispatch`; deletes all Actions artifacts and caches. Safe to delete all because this repo runs no image builds and no `actions/upload-artifact`, so it produces zero artifacts. Prevents `.dockerbuild` build-record accumulation against the org-shared 2 GB GitHub Actions storage quota.
...
| `action-pins` | Verifies every third-party GitHub Action is pinned to a full commit SHA, not a mutable tag — see [infrastructure-overview.md](infrastructure-overview.md#ci-pipeline) | Yes |
| `tofu-validate` | `tofu fmt -check` + `init -backend=false` + `validate` on `tofu/unifi` — `init` also verifies every provider hash against the committed `.terraform.lock.hcl` | Yes |
...
| `grafana-alert-uids` | Verifies every provisioned Grafana alert rule/contact-point `uid` is ≤40 characters and unique — a longer UID crash-loops the whole Grafana pod at provisioning time, which `kustomize`/`kubeconform` can't see (#1081); also runs `check-grafana-nodata-state.sh` so `absent()`-based rules use `noDataState: OK` rather than alerting when their watched object is deliberately absent | Yes |
...
| `migration-secret-rbac` | Verifies every Secret a `migrations/*.sh` script reads with `kubectl get secret` has a matching `resourceNames` + `get` grant in `migrations/rbac.yaml`, and that no `get` grant is orphaned (granted but referenced by no migration, widening the SA's blast radius for nothing — #1161) — RBAC denial returns Forbidden, not NotFound, so a missing grant makes the script's idempotency guard silently inoperative (#923) | Yes |
...
| Task | Description |
|------|-------------|
| `task validate` | Full CI pipeline locally (lint → kustomize → kubeconform → check-completeness → trivyignore-check → check-ingress-uniqueness → check-image-consistency → check-gate-netpol → check-image-pull-secrets → check-grafana-alert-uids → check-grafana-nodata-state → check-migration-exec-secrets → check-migration-secret-rbac → check-kubesec → check-action-pins → tofu-validate) |
...
Configured via `renovate.json` (detects `HelmRelease` resources through the `flux` manager and container images through the `kubernetes` manager, plus six regex `customManagers` for CI tool versions, Flux CRD schema URLs, and the Flux GitHub Action version) and runs self-hosted via `.github/workflows/renovate.yml` on `[self-hosted, linux]`, weekly plus `workflow_dispatch` (with `dry_run` and `log_level` inputs), authenticated with a dedicated repo-level `RENOVATE_TOKEN` PAT secret (separate from `FLEET_INFRA_VERSION_BUMP`; needs Issues, Workflows, Commit statuses and Dependabot-alerts permissions). See [infrastructure-overview.md](infrastructure-overview.md#automated-dependency-updates) for the full setup. Minor, patch, pin and digest updates carry the `Automerge` label, so Claws merges them once CI is green and its review is clean; major updates keep `major-update` and still wait for a human, and `tofu` updates always need a human merge regardless of label.
...
- [Shared PostgreSQL](postgres.md) — The single `apps/postgres` instance behind Authentik, Immich, Garden, Forgejo, Claws and growth-engine: image choice, adding a database, NetworkPolicy allow-list, backups
...
- [Forgejo](forgejo.md) — Self-hosted Git forge, SQLite/PVC layout, upgrade runbook, Actions OIDC and ephemeral runners, the archived-GitHub-copy policy and Forgejo-side Renovate
...
- [Forgejo Actions](forgejo-actions.md) — In-cluster forgejo-runner with a Docker-in-Docker sidecar, offline registration, runner labels and Docker access for job containers
- [Shared Browser Service](browser.md) — Off-the-shelf browserless Chromium with a concurrency cap and queue, shared by Claws and growth-engine
- [UniFi Network](unifi.md) — Declarative UniFi config via tofu-controller: the IoT VLAN, zone firewall policies, the UI-is-read-only house rule, supply-chain posture
