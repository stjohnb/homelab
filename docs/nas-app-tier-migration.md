...
**Depth:** **Deep dive**
**Read this when:** you need the history of why Plex/Jellyfin run on `k3s` behind soft NFS instead of the NAS node, or are repeating a similar cutover.
**Read instead:** [nas-k3s.md](nas-k3s.md) for current NAS node behaviour.
...
- **Jellyfin:** now **enforced automatically** by the `config-reconciler` sidecar
  ...
  and `SaveTrickplayWithMedia` to `false` on every library. Since #1075 the
  same sidecar also *starts* the scan — every 5 minutes it checks TCP
  192.168.0.128:2049 and that `/media/Movies` and `/media/TV` are non-empty,
  and starts the `RefreshLibrary` task at most once an hour (rate limit read
  from the task's own `LastExecutionResult`, so a manual Dashboard scan also
  backs it off). Both checks are required; skipping is silent by design.
  Jellyfin's own triggers stay cleared — the cadence lives in this repo.
  Nothing to do by hand — but it needs the `jellyfin-api-key` Secret to exist:
...
| Their config backup | `config-backup` | `config-backup-players` (02:05, unpinned) |
...
- The players-backup false-positive risk flagged at cutover is closed — the schedule sits inside the NAS's 02:03–03:03 awake window (02:05), the alert filters `reason="DeadlineExceeded"`, and `Config Backup (Players) Job Stale` (14 days) prevents a silently-dead backup hiding forever. See [config-backups.md](config-backups.md#alerting).

- Navidrome (#959) was added directly into the read tier — `k3s` + `media-soft-pvc`, scanner scheduling off, `PurgeMissing=never` — rather than being cut over from `k3s-nas` through this runbook.
- **Jellyfin's scan gate (#1075) depends on two hard-coded checks**: the NFS server answering on 192.168.0.128:2049, and the library root directories being named `/media/Movies` and `/media/TV`. Renaming a library root under `/mnt/SSD-POOL/media` silently disables scanning — the sidecar keeps clearing triggers but never sees the mount as populated.
