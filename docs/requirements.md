...
**Depth:** **Reference**
**Read this when:** proposing something cross-cutting, such as autoscaling, a new auth/CI pattern, or a policy-enforcement gate.
**Read instead:** the owning subsystem doc when the constraint belongs to one; see [OVERVIEW.md](OVERVIEW.md) for the doc map.
...
PR #937 reworded the Forgejo mirroring doc (deleted in #1279; the surviving policy lives in
`docs/forgejo.md`) after an implementer wrote that the push-mirror
