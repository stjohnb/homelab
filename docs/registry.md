...
**Depth:** **Reference**
**Read this when:** working on the self-hosted zot registry or images served from `registry.home.bstjohn.net`.
**Read instead:** [ghcr-auth.md](ghcr-auth.md) for GHCR pull auth.
...
**fleet-infra builds no container images at all** — it only deploys and configures them, receiving tag bumps from each image's own build repo (#1189). Every image it deploys is built elsewhere:

| Image | Built by | Tag scheme | Pinned in fleet-infra |
|---|---|---|---|
| `arpwatch` | `St-John-Software/arpwatch` on `git.home.bstjohn.net`, `.forgejo/workflows/build.yml` | `vYYYY-MM-DD.N` (N from the zot tags API) | `apps/arpwatch/deployment.yaml` |
| `forgejo-runner-nix` | `St-John-Software/forgejo`, `.forgejo/workflows/build-runner-image.yml` | `YYYYMMDD-HHMM-<sha7>` | `apps/forgejo-runner/config.yaml`, `config-beefy.yaml` |
| `forgejo` | `St-John-Software/forgejo`, `.forgejo/workflows/build-image.yml` (see [forgejo.md](forgejo.md)) | `<UPSTREAM_VERSION>-<sha7>-rootless` | `apps/forgejo/deployment.yaml`, `cronjob-backup.yaml` |
| `bin-scraper` | `St-John-Software/bin-scraper`, `.forgejo/workflows/ci.yml` | `YYYYMMDD-N-<sha>` | `apps/bin-scraper/deployment.yaml` |
| `cycling` | `St-John-Software/cycling` on `git.home.bstjohn.net` (release job) | `YYYYMMDD-HHMM-<sha7>` | `apps/cycling/deployment.yaml` |
| `growth-engine` | `St-John-Software/growth-engine` on `git.home.bstjohn.net`, `.forgejo/workflows/build.yml` | `YYYYMMDD-HHMM-<sha7>` | `apps/growth-engine/deployment.yaml` |

All six push **only** to the LAN registry — the GHCR push was dropped once every GitHub Actions runner was on the LAN (#975). A registry push failure now fails the build loudly rather than being skipped. Image tags pushed to `ghcr.io/st-john-software/{arpwatch,forgejo-runner-nix}` before 2026-08-27 are retained, unmaintained, for emergency rollback only.

### Seeding the arpwatch build repo

The Dockerfile and build workflow were removed from this repo in #1189 and are recoverable from git history:

```bash
git show $(git rev-list -1 HEAD -- images/arpwatch/Dockerfile)^:images/arpwatch/Dockerfile
git show $(git rev-list -1 HEAD -- .github/workflows/build-arpwatch.yml)^:.github/workflows/build-arpwatch.yml
```

The new `St-John-Software/arpwatch` repo on `git.home.bstjohn.net` needs:

- The Dockerfile at its root, with `org.opencontainers.image.source` repointed at `https://git.home.bstjohn.net/St-John-Software/arpwatch`.
- `.forgejo/workflows/build.yml` on `runs-on: [self-hosted, linux]`, modelled on the forgejo build repo's `build-image.yml` — docker CLI, buildx plugin, `curl`, `jq` and `gh` all from nix (the job image ships none of them but does have `/var/run/docker.sock` from the dind sidecar), OCI push via `--output type=image,oci-mediatypes=true,push=true` on a `docker-container` builder named per run and removed in `if: always()`, `--driver-opt network=host`.
- **No repo secrets** — it reads the org-level Forgejo Actions secrets `REGISTRY_USER`/`REGISTRY_PASSWORD` and `FLEET_INFRA_VERSION_BUMP`.
- A final `gh workflow run update-arpwatch.yml --repo St-John-Software/fleet-infra --field tag=<tag>`.
- `claws.json` = `{"enabled": true}`.

Two contracts must survive verbatim: the IEEE OUI bake (`curl https://standards-oui.ieee.org/oui/oui.txt` → awk → `/usr/share/arpwatch/ethercodes.dat`, #570) and the `ln -s /usr/local/bin/notify-slack.sh /usr/sbin/sendmail` shim, whose target is supplied at runtime by the `arpwatch-config` ConfigMap in `apps/arpwatch/config.yaml`. **No GitHub push mirror** — Forgejo→GitHub mirrors are retired (#1279).

### Seeding the runner image in the forgejo build repo

Move `images/forgejo-runner-nix/Dockerfile` (recoverable the same way as above) to `runner-image/Dockerfile` in `St-John-Software/forgejo`, with its source label repointed at that repo, and add `.forgejo/workflows/build-runner-image.yml` — the same builder recipe as `build-image.yml`, triggered on push to `main` path-filtered to `runner-image/**` plus `workflow_dispatch`, tag `$(date -u +'%Y%m%d-%H%M')-<sha7>` plus `latest`, dispatching `update-forgejo-runner-nix.yml`. Preserve: nix on `PATH` via `ENV` (act_runner uses a non-login bash, so `/etc/profile.d` is never sourced) and `/etc/nix/nix.conf` with empty `build-users-group` + `sandbox = false` (job containers are unprivileged). Routine rebuilds are fine on Forgejo Actions — the running job uses the previous tag.
...
|---|---|---|
| `ci` (read + write) | `registry-auth` Secret — an htpasswd line plus the plaintext `ci-username`/`ci-password` keys | Forgejo Actions push from the build repos (`arpwatch`, `forgejo`, `bin-scraper`, `cycling`, `growth-engine`) using **org-level** `REGISTRY_USER`/`REGISTRY_PASSWORD` secrets on `St-John-Software` in Forgejo; no repo-level copies |
...
Blobs are deliberately **not** backed up. Every image here is reproducible from its build repo (see the table above); recovery from a lost PVC is re-running the build workflows.
...
3. Read the new `ci-password` and update the **org-level** `REGISTRY_USER`/`REGISTRY_PASSWORD` Forgejo Actions secrets on `St-John-Software` by hand — until you do, every build repo (`arpwatch`, `forgejo`, `bin-scraper`) fails at the login step, and arpwatch's also fails at its tag-computation step (HTTP 401 from the tags API). fleet-infra's own GitHub `REGISTRY_USER`/`REGISTRY_PASSWORD` repository secrets are unused after #1189 and should be deleted.
...
- On a cluster rebuild the forge itself waits on the shared Postgres instance and on migrations `0024` (superuser Secret) and `0030` (the `forgejo` database), so the order is postgres → migrations → forgejo → forgejo-runner → registry-consuming workloads (#1216).
- On a cluster rebuild the PVC is empty, and **none** of these images can be rebuilt by Forgejo Actions, because the forge and its runners are themselves down and `forgejo-runner-nix` is the job image those runners use. Order: (1) start Forgejo on upstream `codeberg.org/forgejo/forgejo:15.0.x-rootless`, or rebuild the fork image with the one-off privileged `moby/buildkit` Job in [forgejo.md](forgejo.md) "Self-built image" (keep `oci-mediatypes=true`); (2) rebuild `forgejo-runner-nix` from `runner-image/Dockerfile` in the same build repo with the same BuildKit Job; (3) once the fleet is up, dispatch the arpwatch build normally on Forgejo Actions. The build repo's GitHub copy is archived and private since 2026-09-10 and its mirror is gone (#1279), so the break-glass clone source is a local clone, then that archived snapshot (authenticated, possibly stale), then the newest `forgejo-dump-*.tar.gz` on the NAS at `/mnt/SSD-POOL/media/backups/forgejo/` — extract `repos/st-john-software/forgejo.git` and clone it (`docs/forgejo-backups.md`).
