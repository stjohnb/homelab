...
**Depth:** **Reference**
**Read this when:** working on Flux's Slack alerting or Grafana deployment annotations.
**Read instead:** [monitoring.md](monitoring.md) for CronJob-specific alert rules.
...
- **New Kustomizations**: add every new top-level Kustomization (e.g. `claws-sessions`) to `slack-errors`'s `eventSources` so failures surface in Slack.
