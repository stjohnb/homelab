...
|---|---|---|
| `ci` (read + write) | `registry-auth` Secret — an htpasswd line plus the plaintext `ci-username`/`ci-password` keys | Forgejo Actions push from the build repos (`arpwatch`, `forgejo`, `bin-scraper`) using **org-level** `REGISTRY_USER`/`REGISTRY_PASSWORD` secrets on `St-John-Software` in Forgejo; no repo-level copies |
