...
- Treat [`migrations/`](migrations/) as automatic. Merged migration changes are picked up by Flux; do not describe "run the migration" as a manual step. The same holds for anything else Flux performs on merge — see the manual-action rule (hard invariant 10) in [`.agents/issue-refiner.md`](.agents/issue-refiner.md).
