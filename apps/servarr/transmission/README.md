...
- **Downloads PV**: `192.168.0.128:/mnt/SSD-POOL/downloads` (1Ti, `transmission-downloads-pvc`) —
...
### 4. Transmission Credentials (✅ Complete)
Credentials stored in Kubernetes secret `transmission-auth`:
```bash
kubectl get secret transmission-auth
```
Username and password referenced in deployment via `secretKeyRef`.
...
Credentials stored in `transmission-auth` secret:
```bash
# View credentials
kubectl get secret transmission-auth -o jsonpath='{.data.username}' | base64 -d
kubectl get secret transmission-auth -o jsonpath='{.data.password}' | base64 -d
```
...
2. **Restrict web UI access** to your LAN subnet only
3. **Enable authentication** for RPC (already enabled via USER/PASS)
4. **Keep WireGuard keys secure** (stored only in the `transmission-wireguard` secret)
5. **Regular updates** of container images
6. **Monitor bandwidth** for anomalies
7. **Use strong passwords** for Transmission web UI
8. **Audit NetworkPolicy** regularly

## References

- Todo & Plans: `../TODO.md`
- WireGuard config source: Mullvad account page → Linux / WireGuard (ie-dub)
- Secrets: `transmission-wireguard`, `transmission-auth` (both created imperatively via kubectl)
