...
- **`immich-postgres-pvc`** — deliberately excluded. It is covered by the 6-hourly logical `pg_dump` in `apps/immich/cronjob-db-backup.yaml`. A hot `tar` of a live PostgreSQL data directory is worse than useless: it looks like a backup and will not restore. A correct physical backup needs `pg_basebackup` and WAL archiving, which this cluster does not have.
