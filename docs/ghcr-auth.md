...
**Reference.** Read this when debugging GHCR pull auth failures or wiring a pull secret for a new service. For the self-hosted registry alternative, see [registry.md](registry.md).
...
Images this organisation builds itself are moving off GHCR to the self-hosted zot registry at `registry.home.bstjohn.net` (see [registry.md](registry.md)); once that migration completes, `ghcr-pull` serves only `bin-scraper` and `claws`.
