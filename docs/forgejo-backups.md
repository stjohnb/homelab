...
**Depth:** **Deep dive**
**Read this when:** restoring Forgejo from backup or changing the local/offsite backup CronJobs.
**Read instead:** [forgejo.md](forgejo.md) for Forgejo's general architecture.

Operator runbook for the Forgejo backup CronJobs and the restore procedure.

Forgejo (`git.home.bstjohn.net`, `apps/forgejo/`) stores its repositories, LFS objects, JWT and SSH host keys, and `app.ini` on a single 10 Gi `local-path` PVC (`forgejo-data`) on node `k3s`; since #1216 the database is the `forgejo` database on the shared `apps/postgres` instance ([postgres.md](postgres.md)). As it becomes the canonical forge for repos migrated off GitHub, that PVC needs a backup story. The nightly `postgres-db-backup` job covers the database only — it does **not** replace the CronJobs below, which are the sole backup of the repository data (and also carry a logical copy of the database).
...
**Stage 1** runs `forgejo dump --type tar.gz` in a second pod using the same image tag (a hard requirement, not a convention) and the same `forgejo-data` PVC as the running server, and writes the archive to a dedicated `local-path` PVC. It mounts no NFS, so nothing external can block it. It must succeed every night. Forgejo image bumps merge unattended (see the Upgrade Runbook in [forgejo.md](forgejo.md)), so the newest Stage 1 dump is the restore point for a failed upgrade migration — one more reason it must succeed every night.
...
- **Pod exited 137 with `reason: OOMKilled`.** The copy itself ran out of memory. `cp` to NFS leaves the copied file as dirty page cache charged to the pod's memory cgroup, and NFS has no cgroup-writeback support, so the cgroup OOMs instead of throttling the writer. Observed 2026-09-10 on `forgejo-backup-offsite-29816760-n4zz9` at a 256Mi limit, once dumps grew from ~88 MB to ~302 MB as repos migrated off GitHub. `backoffLimit: 1` retried and the retry succeeded, so the **Job read `Complete` while a `Failed` pod was retained** — that retained pod is what the Claws pod-failure watcher reports (issue #1261). The job now copies in 64 MiB `dd ... conv=fsync` chunks, which bounds dirty pages independently of dump size, at a 512Mi limit.
...
Dump size is not static: archives were ~88 MB through 2026-09-06 and ~302 MB from 2026-09-07 as repositories migrated off GitHub. At 14-dump retention that is ~4.2 GB per stage. The `forgejo-backups` `local-path` PVC's 5 Gi request is nominal — `local-path` enforces no quota, so the real ceiling is free space on node `k3s` (45 GB free of 124 GB as of 2026-09-10), and the `local-path` StorageClass leaves `allowVolumeExpansion` unset, so the PVC request **cannot** be edited in place. If dumps keep growing, cut retention in Stage 1's `tail -n +15` rather than trying to resize the PVC.

## Archive layout

A `forgejo dump` tar.gz contains, at the top level:

- **`forgejo-db.sql`** — logical SQL dump of the database, in Postgres dialect since #1216. **This is the authoritative copy of the database within the archive.** (Note the name: `forgejo-db.sql`, not `gitea-db.sql`.) On an empty forge it is roughly 74 KB.
- **`app.ini`** — copy of the configuration.
- **`data/`** — the whole work directory, including `data/custom/conf/app.ini`, `data/jwt/private.pem`, `data/ssh/gitea.rsa` and `data/indexers/`. A `data/data/gitea.db` may still be present: it is the pre-cutover SQLite file kept as the rollback copy (see [forgejo.md](forgejo.md) "Database") and is stale.
- **`repos/`** — bare repositories. This appears only once repos exist; a forge with zero repos produces no `repos/` entry.

**Never restore the database from `data/data/gitea.db`.** It stopped being written at the Postgres cutover. Always restore the database from `forgejo-db.sql`, or from a `postgres-db-backup` `.pgdump` (`/media/backups/postgres/forgejo/`).

Stage 1 waits for the database port before dumping. Since the database moved to `apps/postgres` (#1216) a brand-new pod's first connection is rejected for about a second while k3s's network-policy controller adds the pod's IP to the `postgres-ingress-restricted` allow-list; `forgejo dump` connects exactly once, so without the wait the first two runs after the cutover (2026-09-09) failed with `connection refused` and `backoffLimit: 1` gave up. The wait is bounded (30 s), so a database that is really down still fails the job loudly.
...
- There is no longer a GitHub push mirror as a second live copy — Forgejo→GitHub mirrors were retired in #1279 and the remaining GitHub copies are archived, read-only and frozen at their last mirrored commit (see "GitHub copies" in [forgejo.md](forgejo.md)). These dumps are the only current off-node copy of repo content, and always were the only copy of issues, PRs, users and settings. Break-glass clone of a single repo: extract `repos/<owner>/<repo>.git` from the newest tarball and `git clone` it.
...
6. **Replace the database.** The database lives on `apps/postgres`, so this step runs against the postgres pod, not the helper pod. Either source works:

   ```sh
   # (a) from the forgejo dump archive — Postgres-dialect SQL
   kubectl exec -i -n default deploy/postgres -- sh -c 'cat > /tmp/forgejo-db.sql' < /restore/forgejo-db.sql
   # inside the postgres pod, as the superuser (PGPASSWORD=$POSTGRES_PASSWORD):
   #   psql -h 127.0.0.1 -U "$POSTGRES_USER" -d postgres -c 'drop database if exists forgejo with (force)'
   #   psql -h 127.0.0.1 -U "$POSTGRES_USER" -d postgres -c 'create database forgejo owner forgejo'
   # then as the forgejo role (password from the forgejo-db-secret Secret, on stdin, never argv):
   #   psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -U forgejo -d forgejo -q -f /tmp/forgejo-db.sql
   # (the dump's tables are not in foreign-key order — load them the way
   # migrations/0030-forgejo-db.sh does: CREATE TABLE lines in passes, indexes,
   # then rows under SET session_replication_role = replica as the superuser)
   # and re-run the serial-sequence reset DO block from migrations/0030-forgejo-db.sh —
   # xorm's dump never advances the sequences, and without it the first write fails
   # with "duplicate key value violates unique constraint".

   # (b) from a postgres-db-backup archive
   #   drop/create the database as above, then
   #   pg_restore -h 127.0.0.1 -U forgejo -d forgejo /media/backups/postgres/forgejo/<file>.pgdump
   ```
...
Both Forgejo CronJobs carry `ttlSecondsAfterFinished: 172800` (added in #1238), so a finished Job — succeeded or failed — and its pods self-delete after 48 hours. That window is deliberately long enough to diagnose a nightly failure the next morning; do **not** shorten it, and do **not** lower `failedJobsHistoryLimit`, because either would delete the pod logs these incidents are diagnosed from. The TTL is not retroactive: Jobs created before #1238 have an empty `.spec.ttlSecondsAfterFinished` and persist until deleted by hand.
