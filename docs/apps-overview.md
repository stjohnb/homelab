...
**Depth:** **Reference**
**Read this when:** adding a new service to `apps/`, or checking existing services, service types, Headlamp, or migrations conventions.
**Read instead:** [infrastructure-overview.md](infrastructure-overview.md) for Traefik, cert-manager, Flux, RBAC, or CI.
...
│
├── forgejo/                    # Git forge (git.home.bstjohn.net) + 2 backup CronJobs — see docs/forgejo.md
...
├── datasette/                  # Datasette over nightly redacted SQLite exports of the Postgres DBs
...
├── renovate/                    # CronJob: weekly dependency-update PRs against Forgejo (see docs/forgejo.md)
...
**Proxmox and UniFi** use Traefik `IngressRoute` + `ServersTransport` instead of standard Ingress, because their backends serve self-signed HTTPS. Proxmox pins the PVE cluster CA (`rootCAsSecrets: [proxmox-root-ca]`, from `apps/proxmox/root-ca.enc.yaml`); UniFi still uses `insecureSkipVerify: true`.
...
The ForwardAuth address uses FQDN `authentik-server.default.svc.cluster.local` because Traefik runs in its own namespace. Two outposts are declaratively configured in the blueprint — the embedded outpost carries the 13 `.home.` providers and the standalone `ext-proxy-outpost` (`apps/authentik/deployment-proxy-ext.yaml`) the 12 `.ext.` ones, so ext logins redirect to `auth.ext.bstjohn.net` (#1111); each `authentik_outposts.outpost` entry lists its providers by `!KeyOf` reference, ensuring provider-to-outpost attachment is version-controlled. Proxy providers, applications, users, groups, and per-app policy bindings are all configured declaratively via Authentik blueprints (`configmap-blueprints.yaml`), which define 25 ForwardAuth proxy providers (one home and one ext per service, except Datasette which is home-only), 25 proxy applications, 12 OIDC providers (+12 provider-less ext bookmarks), 4 groups (`all-apps`, `media`, `home`, `infra`), 2 users, and 74 policy bindings. Each application is bound to a category-specific group plus the `all-apps` fallback group using `policy_engine_mode: any`. Group memberships are fully declarative — manual changes in the Authentik UI are reverted on reconciliation. Gatus is intentionally excluded from ForwardAuth so that monitoring remains accessible during an Authentik outage. Grafana additionally uses `auth.proxy` for automatic user creation from Authentik sessions. See [docs/authentik.md](authentik.md) for the full architecture, blueprint configuration, access control model, protected service list, and security model.

### Ingress (Home and Ext Domains)

All services use `*.home.bstjohn.net` (LAN, 192.168.0.251) ingresses, and each one carries a sibling `*.ext.bstjohn.net` host on the same resource — ForwardAuth services are the exception: their ext host lives in a sibling `<svc>-ext` Ingress because it needs the other middleware chain (#1111). Every other service carries both hosts on one resource. Tailscale split DNS sends the whole `bstjohn.net` domain to the in-cluster `tailnet-dns` resolver at `100.78.7.18` (`apps/tailnet-dns/`), which answers `100.78.7.18` for `home.bstjohn.net` and every subdomain and forwards the rest of the domain upstream, so `.home` reaches Traefik with no subnet router; the openclaw subnet router remains the fallback for encrypted-DNS clients. `*.ext` resolves to the `k3s` node's tailnet address (100.78.7.18) and reaches Traefik with no subnet router involved either. See [Remote access over Tailscale](infrastructure-overview.md#remote-access-over-tailscale).
...
Arpwatch monitors ARP traffic on the host network to detect new devices and address changes. It uses a dedicated container image (`registry.home.bstjohn.net/st-john-software/arpwatch`) built by `St-John-Software/arpwatch` on `git.home.bstjohn.net` with calver tags (`vYYYY-MM-DD.N`) — see [registry.md](registry.md). The image bundles arpwatch, jq, curl, and the full IEEE OUI vendor database (`ethercodes.dat`, downloaded from `standards-oui.ieee.org` at build time via awk) — this ensures vendor lookups work correctly without relying on Alpine's bundled stub. It runs with `-F` (foreground) mode which keeps the process as PID 1 while still invoking sendmail for alerts. The sendmail binary is replaced by a symlinked shell script (`notify-slack.sh` from a ConfigMap) that formats alerts as Slack messages using `jq` for JSON construction. The Slack webhook URL comes from the shared `gatus-secrets` secret. Requires `NET_RAW` capability for ARP monitoring (not fully privileged). The script silently drops alerts where the source IP is `0.0.0.0` — these are normal ARP probes (RFC 5227) and DHCP discovery packets that generate harmless "flip flop" noise when multiple devices probe the network.

### Tailnet DNS

`apps/tailnet-dns/` runs CoreDNS `1.14.7`, authoritative for `home.bstjohn.net` (answers `100.78.7.18`), so off-LAN Tailscale devices reach `.home` services without the openclaw subnet router (#1131), and `wall.bstjohn.net` (answers the prod cluster's tailnet address `100.115.153.16`), so tailnet devices reach the office wall dashboard on the Hetzner prod cluster without tripping its Traefik `ipAllowList` (#1257), and it forwards the rest of `bstjohn.net` to public resolvers (`1.1.1.1` / `1.0.0.1` / `9.9.9.9`) because the Tailscale split-DNS route is now the whole domain — a single restricted-domain entry per nameserver — so `grafana`, `plausible`, `www`, `ext` and the apex `MX`/`TXT` would otherwise be `REFUSED` on every tailnet device (#1264); the catch-all `.:5353` still `REFUSED`s everything outside `bstjohn.net`. Pinned to the `k3s` node via `nodeSelector: kubernetes.io/hostname: k3s` and bound only to its tailnet address: `hostPort: 53` UDP+TCP with `hostIP: 100.78.7.18`, while the container itself listens on unprivileged `5353`. The container keeps `capabilities: drop: [ALL]` but must re-add `NET_BIND_SERVICE`: the upstream image's `/coredns` binary is built with `setcap cap_net_bind_service=+ep`, and under `allowPrivilegeEscalation: false` the kernel refuses to exec it with an empty permitted set — `exec /coredns: operation not permitted` at startup (#1138). Uses `strategy: Recreate` because two pods cannot hold the same hostPort. No Service and no Ingress — nothing routes to it in-cluster. Monitored by the `Tailnet DNS`, `Wall DNS`, and `Tailnet DNS Forward` Gatus checks (`apps/gatus/config.yaml`). The Tailscale split-DNS nameserver pointing at `100.78.7.18` is a manual admin-console setting, not tracked in Git — see [infrastructure-overview.md](infrastructure-overview.md#remote-access-over-tailscale).

### Garden

Garden (`St-John-Software/Garden`) is a Next.js + Prisma app for garden planning, image `registry.home.bstjohn.net/st-john-software/garden`, private and pulled with `imagePullSecrets: [registry-pull]`. Its database lives in the shared PostgreSQL instance — see [postgres.md](postgres.md) — with schema owned entirely by Prisma migrations, applied by an `initContainer` running `npx prisma migrate deploy` from the same image on every rollout, so there is never a manual migration step. There is no S3-compatible object store in this cluster, so photo uploads use `PHOTO_STORAGE_DRIVER=local` with bytes written to a `local-path` PVC (`garden-photos`, 20Gi, pinned to `k3s`) mounted at `PHOTO_LOCAL_ROOT=/var/photos`; only the storage key is persisted in Postgres, so a later move to an `s3` driver needs a file copy, not a data migration. `local` is explicitly an interim choice, not the destination (#1006, #1010): Garden was designed object-storage-first (`s3` driver via `@aws-sdk/client-s3`, works with MinIO/Garage/R2), and the owner's stated direction is to deploy an in-cluster S3-compatible object store (Garage/MinIO/SeaweedFS) and cut Garden over to it — both because that was the intended architecture and because an object store would also be the natural offsite/backup target the cluster otherwise lacks. Until then, `garden-photos` has **no backup** of any kind — unlike every other stateful volume in the cluster, a lost `k3s` node disk loses every uploaded photo with no recovery path; this is accepted as non-urgent only because the app has negligible real photo traffic so far, not as a permanent stance. `.github/workflows/update-garden.yml` mirrors `update-bin-scraper.yml` for the image-bump CD handshake. Authentication is native OIDC inside the app (client `garden`, issuer `https://auth.home.bstjohn.net/application/o/garden/`, redirect `https://garden.home.bstjohn.net/auth/callback`), bound to the `home` and `all-apps` Authentik groups; the client secret is `authentik-secrets`/`GARDEN_OIDC_CLIENT_SECRET` and the session HMAC key is `garden-session`/`session-secret` (#1037). The Ingress carries no ForwardAuth middleware by design — that would break the bearer-token path and put a redirect in front of `/api/health`, which stays open for the kubelet and Gatus probes. Garden's `/seeds/import` seed-packet OCR calls the in-cluster Ollama at `SEED_OCR_URL=http://ollama:11434` with `SEED_OCR_MODEL=minicpm-v` and `SEED_OCR_TIMEOUT_MS=180000`; the model is preloaded declaratively by the `model-preload` container in `apps/ollama/deployment.yaml` (see `docs/gpu-k3s.md`).

### Cycling

Cycling (`St-John-Software/cycling`) is a static Vite/three.js ride-tracking app that talks to a Schwinn 800IC over Web Bluetooth, image `registry.home.bstjohn.net/st-john-software/cycling`, private and pulled with `imagePullSecrets: [registry-pull]`. Web Bluetooth only works in a secure context, which is the whole reason this needs the shared wildcard HTTPS ingress — it is unusable over plain HTTP from a phone or laptop. There is no backend and no PVC: ride history lives entirely in browser storage, so it is per-device and, because `.home` and `.ext` are separate origins, per-host too — a ride recorded on `cycling.home.bstjohn.net` does not appear on `cycling.ext.bstjohn.net`. The Ingress carries no Authentik ForwardAuth by design: the app serves no server-side data and both hosts are LAN/tailnet-only, and adding ForwardAuth would make an Authentik outage break the app mid-ride. Probes, the Gatus check and the Homepage tile all hit `GET /`, since the underlying nginx image always returns `index.html` with 200 rather than a dedicated health endpoint. `.github/workflows/update-cycling.yml` mirrors `update-garden.yml` for the image-bump CD handshake.

### growth-engine

growth-engine (`St-John-Software/growth-engine`, Forgejo) is the marketing/promotion automation service, image `registry.home.bstjohn.net/st-john-software/growth-engine`, private and pulled with `imagePullSecrets: [registry-pull]`. Its database lives in the shared PostgreSQL instance (#1368) — see [postgres.md](postgres.md) — with schema applied by an `initContainer` running `ge migrate` from the same image on every rollout, the same shape as Garden's Prisma init container. `DATABASE_URL` is built inline as `postgres://growth_engine:$(GROWTH_ENGINE_DB_PASSWORD)@postgres.default.svc.cluster.local:5432/growth_engine`, with `GROWTH_ENGINE_DB_PASSWORD` defined earlier in the same `env:` list so kubelet's `$(VAR)` expansion can see it — growth-engine uses node-postgres, which does **not** merge a separately supplied password into `connectionString` (#1299), so a passwordless URL plus a sibling password field would crash-loop instead. The password is alphanumeric-only by construction, so it needs no escaping in the interpolated form.

The single exception to "no platform credentials in SOPS or deployment secret stores" (growth-engine's own product docs) is `GROWTH_ENGINE_MASTER_KEY`: it encrypts every stored platform credential in the database, so it has to exist before the app can decrypt anything, and it lives in a SOPS-encrypted Secret, `apps/growth-engine/master-key.enc.yaml`. The age private key that decrypts it is therefore the only backup of that key — losing it loses every stored credential, not just an inconvenience.

The Deployment ships with `replicas: 0` and a placeholder image tag, because no image exists yet (growth-engine#8). `.github/workflows/update-growth-engine.yml` is the image-bump receiver; its first run also flips `replicas: 0` to `1` by matching the exact marker comment on that line, so the bootstrap resolves itself with no manual edit once growth-engine publishes its first build. The dashboard sits behind Authentik ForwardAuth on both `growth-engine.home.bstjohn.net` and `growth-engine.ext.bstjohn.net`, restricted to the `infra` group like the other admin-only dashboards (Grafana, Prometheus, Datasette).

### Browser

`apps/browser/` runs one shared `ghcr.io/browserless/chromium` Deployment, pinned to node `k3s`, so Claws' `@playwright/mcp` sessions and growth-engine's headless Chromium share a single concurrency cap and wait queue instead of running uncoordinated Chromium processes on the same node. No Ingress — it's in-cluster only, reached via the `browser` ClusterIP Service on port 3000 and gated by a NetworkPolicy allow-list. See [browser.md](browser.md) for the endpoint URLs, the queue tuning, and the verification recipe.
...
| `0032-renovate-forgejo-token.sh` | `renovate-forgejo-token` Secret (`token` key) — creates a site-admin Forgejo user `renovate` and mints a token scoped `write:repository,write:issue,read:user,read:organization` (no admin scope) via `forgejo admin user create` / `generate-access-token` exec, fed straight to a Secret without ever appearing as an exec argument (#902); see [forgejo.md](forgejo.md#dependency-updates-renovate) |
| `0034-claws-forgejo-accounts.sh` | `claws-forgejo-tokens` Secret (`service-token`, `admin-token`) — ensures the non-admin Forgejo users `clawsstjohn` and `claws-admin` exist, puts them in the org teams `claws-bot` and `Owners` via a throwaway site-admin user deleted in the same in-pod shell, and mints one token each; nothing ever appears as an exec argument (#902); see [forgejo.md](forgejo.md#claws-bot-accounts) |
| `0035-growth-engine-db.sh` | Creates the `growth_engine` role and empty database on the shared PostgreSQL instance and the `growth-engine-db-secret` Secret (`password` key), same shape as `0033`; schema is owned by growth-engine's own migrations |
| `0036-browser-token.sh` | `browser-token` Secret (`token` key) — bearer token for the shared browserless service, same shape as `0020`; see [browser.md](browser.md) |
...
`apps/claws/` contains both an external proxy (`claws.home.bstjohn.net` → `192.168.0.73:3000`) and the staging deployment's Service, Ingress, RBAC and SOPS Secrets (`claws-staging.home.bstjohn.net`). It no longer holds the staging StatefulSet itself: since #1373 that lives at `clusters/my-cluster/claws-staging/statefulset.yaml` in its own Flux Kustomization (`claws-staging`), gated on a pre-puller Deployment at `clusters/my-cluster/claws-staging-prepull/deployment.yaml` (Flux Kustomization `claws-staging-prepull`) — see "Image updates are automatic" below.

The staging workload is a StatefulSet (`claws-staging`) with a `volumeClaimTemplate` (50Gi `local-path`, PVC `data-claws-staging-0`), pinned to node `k3s` via `nodeSelector: kubernetes.io/hostname: k3s` — `local-path` volumes are node-local and the other two nodes are powered off most of the day, so a restart always finds its data. The StatefulSet replaced an earlier Deployment on a fixed 1Gi `claws-staging-data` PVC, which could not be grown in place because the `local-path` StorageClass has no `allowVolumeExpansion`; that old claim was carried through the swap with `kustomize.toolkit.fluxcd.io/prune: disabled` so the merge could not delete it, then removed from Git and deleted by hand once the 50Gi volume was verified (#1061). Note that `local-path` enforces no quota, so a runaway `repos/`+`worktrees/` tree on the 50Gi claim could trigger node-wide DiskPressure evictions on the control-plane node. The `state/` and `cache/` subPath mounts described below (#1338) sit on this same unquota'd claim, and since files on a PVC fall outside the pod's `ephemeral-storage` accounting, none of that growth counts against the container's node-local overlay quota either. `data-claws-staging-0` is `local-path`, so `/home/claws/.claws` is really just a directory on node `k3s`'s root filesystem — `/dev/sda1`, 125G, about 69% used as of 2026-09-14, of which about 30G is containerd. The PVC, the container's writable overlay and the `/tmp` `emptyDir` below therefore all draw on the same disk; the 50Gi claim is not reserved capacity. A cache PVC would add persistence across image bumps, not capacity, which is why none exists (#1335).

`terminationGracePeriodSeconds: 420` is 300s scheduler drain + 5s task-cancel + 60s memory flush + headroom — changing claws' `shutdown()` flush cap means changing this number. Resources are `requests: {cpu: 500m, memory: 2Gi, ephemeral-storage: 8Gi}`, `limits: {memory: 6Gi, ephemeral-storage: 24Gi}`, deliberately with no CPU limit: a throttled claude worker process stalls agent runs, so the memory limit is a burst backstop rather than a target. `/tmp` is mounted as a `12Gi` `emptyDir` for agent scratch. The 24Gi ephemeral-storage limit bounds the container's writable overlay plus `/tmp` together, sized from openclaw's worst day (~14G of scratch, claws#3016), but it does **not** bound the PVC, whose `repos/`+`worktrees/` growth is still unquota'd. Exceeding either bound is an immediate kubelet eviction that skips the 420s graceful drain — acceptable for a backstop since the state that matters (`~/.claws` on the PVC, the database in Postgres) is untouched; only in-flight agent runs and unflushed memory are lost. To read a capacity alert: `Evicted` with `ephemeral local storage usage exceeds` or `usage of EmptyDir volume "tmp" exceeds` means Claws scratch hit its bound — check `/tmp`, `~/.claude` and `~/.local/share` in the pod. Node `DiskPressure` or `NodeFilesystemAlmostFull` on `k3s` means the shared disk overall, which includes the PVC and containerd.
...
**Credential rotation without a PR.** `/claude-auth` persists a refreshed `CLAUDE_CODE_OAUTH_TOKEN` to `~/.claws/env` on the PVC, and that file wins over the Secret on the next boot (`loadEnvFile()` in claws' `src/env-file.ts`). Codex rotation (claws#2802, shipped) works the same way from the dashboard's `/reauth` tab (`codex login --device-auth` run in-pod from the UI) rather than `~/.codex/auth.json`, which is still rewritten from `CLAWS_CODEX_AUTH_JSON` on every boot — a rotated ChatGPT refresh token from `/reauth` therefore needs to land back in the `claws-auth` Secret to survive a restart, not just the live pod. The `claws-staging` ServiceAccount and `claws-staging-auth` Role (`apps/claws/rbac.yaml`, `get`/`patch` on `resourceNames: [claws-auth]`) exist so claws can write the rotated value straight back into the Secret itself, via the `auth-secret-sync` job (claws#2796, image ≥ v2026-09-02.13) — the write must use the projected SA token (`--server https://kubernetes.default.svc --token "$(cat /var/run/secrets/kubernetes.io/serviceaccount/token)" --certificate-authority /var/run/secrets/kubernetes.io/serviceaccount/ca.crt`) rather than `~/.kube/config`, which the entrypoint overwrites from `CLAWS_KUBECONFIG`. `auth-secret-sync` returns silently before its `kubectl get` when the pod holds no credential yet, so "nothing logged" is expected until a first login exists. Never copy Codex credentials from openclaw into `claws-auth` while openclaw is still running Claws — Codex rotates its refresh token in place, so two live copies invalidate each other; only a from-pod `/reauth` login is safe.
...
**State is now split between the PVC and Postgres (#1274).** The 50Gi `data-claws-staging-0` PVC still holds `~/.claws/config.json` (including the `githubOwnerAppCredentials.stjohnb` path rewrite described above), `~/.claws/env`, `repos/` and `worktrees/`. The **database** moves to the `claws` database on the shared `apps/postgres` instance, reached via the passwordless `CLAWS_DATABASE_URL=postgres://claws@postgres.default.svc.cluster.local:5432/claws` plus `CLAWS_DATABASE_PASSWORD` from the `claws-db-secret` Secret (key `password`, created by `migrations/0033-claws-db.sh`). Since claws#2974 (image `v2026-09-10.9` and later) claws merges that password into the connection string itself, where it takes precedence over any password embedded in the URL, so the two env vars can be declared in any order. Older images could not do this — `pg` re-parsed `connectionString` over its config and dropped a separately-passed password, crash-looping the pod with `SASL: … client password must be a string` (#1299), which #1300 worked around by interpolating `$(CLAWS_DATABASE_PASSWORD)` into the URL and #1303 reverted. `CLAWS_DATABASE_PASSWORD` is also the contract with claws#2953 and the `kubectl exec`-ed snapshot importer. The running image honours both variables, logs `Database backend: PostgreSQL (CLAWS_DATABASE_URL)` at startup, and starts against an empty `claws` database — fine for `verify-only`, with claws#2954's nightly sync from openclaw's live `claws.db` filling it. The sync target is `stagingDbSyncTarget: default/claws-staging-0` in openclaw's `~/.claws/config.json` (hot-reloaded, not a fleet-infra setting), and the job fires nightly at 01:00 local — ahead of the `postgres-db-backup` CronJob's 02:30 dump, so the backup always captures the freshly synced data. Staging is verify-only, so this sync overwrites any staging-side drift unconditionally; openclaw's legacy `ha_deploy_monitor_state` table has no Postgres-schema counterpart and the importer skips it by design, which is expected, not a migration bug. If `claws-staging-0` crash-loops, `OrderedReady` (the StatefulSet's default pod-management policy) blocks the controller from ever attempting a next-generation rollout of that same ordinal — `kubectl delete pod claws-staging-0` to force it, rather than waiting on a rollout that cannot proceed. The dashboard's `/trigger/:job` endpoint needs an interactive OIDC browser session; there is no bearer-token or API route, so a headless session cannot fire the nightly sync (or any other scheduled job) early — either run the equivalent steps by hand (backup API snapshot → prune → `kubectl exec -i` stream → `node dist/tools/import-sqlite-db.js`) or wait for the 01:00 schedule. **Forgejo credentials** arrive the same way: `CLAWS_FORGEJO_TOKEN` and `CLAWS_FORGEJO_ADMIN_TOKEN` come from the `claws-forgejo-tokens` Secret (`service-token` / `admin-token`, created by `migrations/0034-claws-forgejo-accounts.sh`) — the first is `clawsstjohn`, the implicit credential in every Forgejo session, and the second is `claws-admin`, the opt-in `forgejo-admin` capability from claws#2965 that is the only thing with org-owner rights; see [forgejo.md](forgejo.md#claws-bot-accounts).

**`CLAWS_SESSION_MCP_URL`** is set to `http://claws-staging.default.svc.cluster.local:3000`, the base URL `k8s-pod` session pods use for the per-session `claws-state` MCP server (claws#3056). Claws appends `/mcp/sessions/<id>` and strips any trailing `/`. An empty value means pod sessions start without `claws-state`. Active on staging since #1328 flipped `CLAWS_SESSION_BACKEND=k8s-pod`. Session pods (`claws-workload=session` label in namespace `claws-sessions`) reach this pod on TCP 3000 in a cross-namespace call. No NetworkPolicy selects `app: claws-staging` today, so ingress is currently open; as soon as one is added, it must allow TCP 3000 from the session-pod label to avoid blocking traffic silently — blocked traffic only shows as `claws-state` "failed to connect" inside the session, and the session still starts. Similarly, if a session-pod egress policy is ever added, it must allow DNS (UDP/TCP 53 to `kube-dns`) and TCP 3000 to the Claws pod. Changing the Service name or port means changing this value. `CLAWS_SESSION_NODE_SELECTOR=kubernetes.io/hostname=k3s` pins session pods to node `k3s` for the same node-local `local-path` reason as this StatefulSet's own `nodeSelector`.

**`CLAWS_BROWSER_CDP_ENDPOINT`** points claws-staging at the shared browser service (#1383) using `$(BROWSER_TOKEN)` from the `browser-token` Secret, via an order-dependent, `optional: true` `secretKeyRef`. Claws only uses it from claws#3102 on. Details in [browser.md](browser.md).

**`k8s-pod` session backend (#1328).** claws#3026's `k8s-pod` session backend creates one Pod, Service, Secret and PVC per interactive session through the Kubernetes API, so a Claws image rollout never kills a running session. Its RBAC and network policy live in a dedicated namespace, `claws-sessions`, reconciled by its own Flux Kustomization (`clusters/my-cluster/claws-sessions-kustomization.yaml`, no `targetNamespace`, no `dependsOn`) rather than under `apps/` — see [infrastructure-overview.md](infrastructure-overview.md) for why. The namespace enforces Pod Security `restricted` (`pod-security.kubernetes.io/enforce: restricted`) so Claws' session pods must stay PSA-compliant. `clusters/my-cluster/claws-sessions/rbac.yaml` grants ServiceAccount `default/claws-staging` exactly `pods: {get, list, create, delete}`, `services: {get, create, delete}`, `secrets: {get, create, delete, patch}`, `persistentvolumeclaims: {get, list, create, delete}` in `claws-sessions` — the exact verb set Claws' `/verify` `session-backend` row checks for. A `ghcr-pull` dockerconfigjson Secret (SOPS-encrypted, re-encrypted from the same PAT as `apps/ghcr-pull-secret.enc.yaml` — a PAT rotation now updates both files, see [ghcr-auth.md](ghcr-auth.md)) lets session pods pull `ghcr.io/st-john-software/claws:<tag>`. `clusters/my-cluster/claws-sessions/networkpolicy.yaml` restricts ingress on pods labelled `claws-workload: session` to TCP 7681 from only the Claws pod (`app: claws-staging` in `default`); egress is left open for git clone and provider API access, but if it's ever restricted it must still allow DNS and TCP 3000 to `app: claws-staging` for `claws-state`. The `claws-sessions` Namespace carries `kustomize.toolkit.fluxcd.io/prune: disabled` so removing this layer can never cascade-delete every session PVC — those PVCs land on node `k3s`'s shared root disk (see above), same as `data-claws-staging-0`. Staging now runs with `CLAWS_SESSION_BACKEND=k8s-pod` (#1328 PR 2), flipped once the RBAC above was confirmed live. Existing session rows created on the host `local-tmux` backend before the flip are ignored by the pod backend: they are never listed as live, and resuming one fails with "created on the host tmux backend".

The session PVC's mount root, `/home/claws`, is otherwise world-writable (#1382): k3s's bundled `local-path` provisioner creates every volume dir `mkdir -m 0777`, and kubelet's `fsGroup`/`fsGroupChangePolicy: OnRootMismatch` handling adds the `claws` group and setgid bit but never strips the "other" bits, leaving `drwxrwsrwx`. Session pods can't fix this themselves — PSA `restricted` forbids a root or `CAP_FOWNER` init container, and the pods run as uid 1000 against a root-owned dir, so `chmod`/`chown` fails with EPERM. `clusters/my-cluster/claws-sessions/local-path-config.yaml` overrides the provisioner's `kube-system/local-path-config` ConfigMap so any volume dir whose basename matches `*_claws-sessions_*` is created `0770` instead; kubelet then yields `drwxrws--- root:claws` (`2770`). The mode must never drop to `750`: `OnRootMismatch` only skips the recursive re-chown when the group-rw and setgid bits already match, so losing them makes kubelet re-chown the whole volume — `node_modules` and all — on every mount.

**Session transcripts and the npm cache also persist on the PVC (#1338).** Three `subPath` mounts on the existing `data` claim keep interactive sessions resumable and worktree installs warm across an image rollout, which `update-claws-staging.yml` triggers several times a day: `state/claude-projects` → `/home/claws/.claude/projects` (the Claude CLI's `--continue` transcript store), `state/opencode` → `/home/claws/.local/share/opencode` (opencode's session store), and `cache/npm` → `/home/claws/.npm`. Codex gets no mount: Claws already gives every interactive Codex session its own `CODEX_HOME` under `~/.claws/session-mcp/<sessionId>/codex-home` (`ensureSessionCodexHome()` in claws' `src/session-env-file.ts`), which is on the PVC via the `~/.claws` mount already, so `codex resume --last` already survives a rollout; agent runs use a throwaway `CODEX_HOME` and never resume. Because the image creates `~/.claude` and `~/.local` only at runtime, the `data` claim can't be subPath-mounted directly at `~/.claude/projects` or `~/.local/share/opencode` — the container runtime would create the missing parent root-owned and `container-entrypoint.sh` would crash-loop writing `~/.claude/settings.json`. Three plain `emptyDir: {}` volumes, `claude-home`, `local-home` and `local-share`, are mounted at `/home/claws/.claude`, `/home/claws/.local` and `/home/claws/.local/share` as parents instead, with the PVC subPath mounts nested inside them. `~/.local/share` needs its own emptyDir rather than being a directory inside `local-home`: the runtime would create that intermediate mount-point ancestor root-owned `0755`, so uid 1000 could not create siblings of `opencode/` such as the pnpm store at `~/.local/share/pnpm`. Everything else under `$HOME` — settings, skills, credentials, the Claude/Codex CLI installs themselves — is still rebuilt by the entrypoint every boot, so those emptyDirs being wiped on every pod recreation changes nothing about credential ephemerality. One caveat: opencode keeps its own `auth.json` under `~/.local/share/opencode`; nothing in this pod provisions it today, but a manual `opencode auth login` inside the pod would now persist that credential to the PVC. Growth on `state/` and `cache/` is bounded without new code — the Claude CLI prunes transcripts older than `cleanupPeriodDays` (30 by default), and `host-disk-monitor` already sweeps `~/.npm` under disk pressure.
...
**Image updates are automatic.** `ghcr.io/st-john-software/claws` uses date-versioned tags (`vYYYY-MM-DD.N`) that GHCR periodically prunes; a pinned stale tag used to break the pod with `ErrImagePull`/`ImagePullBackOff` and needed a hand-edited PR (#477→#502, #504→#509, #545). Since #1034 the claws repo's Release workflow dispatches `.github/workflows/update-claws-staging.yml` after each image push, which rewrites the tag in both `clusters/my-cluster/claws-staging/statefulset.yaml` and `clusters/my-cluster/claws-staging-prepull/deployment.yaml` and opens/updates a single PR on `automation/bump-claws-staging` (labelled `dependencies,auto-bump`) for Claws' auto-merger. Tracking latest keeps the pinned tag inside GHCR's retention window, so pruning no longer breaks the pod. Do not "fix" pruning by making the package public: the owner explicitly wants org-built images to stay private while the org moves away from GitHub; solve pull auth or image publication instead.

**Bumps pre-pull the image before the pod rolls (#1373).** Before #1373 every bump took the dashboard down for 70–90 s: the StatefulSet stopped the old pod first, and only then did node `k3s` pull the new ~966 MB image (about 67 s) before the ~15 s start. Now the rollout runs in order. The bump PR merges, and `claws-staging-prepull` rolls first (`maxSurge: 1`, `maxUnavailable: 0`, a `sleep infinity` container with a tiny resource footprint). Its new pod pulls the image, about 67 s for 966 MB, while the old pod keeps the previous image in use. Because that Kustomization has `wait: true`, it turns Ready at the new revision only once the pod is Ready. The `claws-staging` Kustomization `dependsOn` it, and Flux refuses to apply it (`dependency 'flux-system/claws-staging-prepull' revision is not up to date`) until then. It then applies the tag, and the new `claws-staging-0` starts from the cached image (`already present on machine`). The outage that remains is the old pod's graceful shutdown plus the ~15 s start, with the ingress returning 404 in between. Claws stays single-instance throughout. True zero downtime would need leader election or blue/green inside claws itself. The running pre-pull pod also keeps the image in use, so kubelet image GC and `containerd-gc` never prune it. A bad or GHCR-pruned tag fails the `claws-staging-prepull` health check, so the StatefulSet stays on its old, working image and `claws-staging` shows `DependencyNotReady` until a good tag lands. This is intended. Fix it with a new tag, not by bypassing the chain. Both files must always carry the same tag. The bump workflow rewrites them together, and `scripts/check-image-tag-consistency.sh` treats the two directories as one group, so CI fails on drift. `apps` is deliberately in neither chain, so a bad claws tag cannot stall every other app. The ServiceAccount, Secrets and `ghcr-pull` the StatefulSet uses stay in `apps`.
...
| `low-priority` | 100000 | servarr services, bin-scraper, arpwatch, jellyfin, navidrome, jellyseerr, seerr, plex, pve-exporter, forgejo-runner, renovate, datasette, browser |
...
Known secrets (referenced in deployments): `gatus-secrets`, `transmission-wireguard`, `immich-db-secret`, `pve-exporter-secret`, `bin-scraper-mqtt`, `bin-scraper-forge-token`, `bin-scraper-session`, `grafana-admin-secret`, `grafana-slack-webhook`, `flux-slack-webhook`, `authentik-secrets`, `jellyfin-api-key`, `postgres-superuser`, `garden-db-secret`, `garden-session`, `claws-db-secret`, `growth-engine-db-secret`, `browser-token`.

A few secrets are committed as SOPS-encrypted `*.enc.yaml` instead, since Flux needs them present at reconcile time rather than pre-created imperatively: `ghcr-pull` (`apps/ghcr-pull-secret.enc.yaml`, see [ghcr-auth.md](ghcr-auth.md)), `bin-scraper-address` (`apps/bin-scraper/address-secret.enc.yaml`, supplies `SCRAPER_POSTCODE`/`SCRAPER_ADDRESS_MATCH`/`REPORTER_POSTCODE`/`REPORTER_ADDRESS_MATCH` so the scraper's postcode and address-match string aren't hardcoded in a tracked file), and `growth-engine-master-key` (`apps/growth-engine/master-key.enc.yaml`). **`SCRAPER_ADDRESS_MATCH` must stay set**: the upstream scraper falls back to picking a random address option from the council site's dropdown when it's unset, which can land on a non-selectable placeholder and fail the scrape with a `waitForSelector` timeout — this caused a real outage (#906) before the key was added to the deployment (#907).
...
| `wall.bstjohn.net` | 100.115.153.16 (tailnet split DNS) / 178.104.218.151 (public) | Office wall dashboard grid on the Hetzner prod cluster; split DNS keeps tailnet devices inside the app's ipAllowList |
...
| Datasette | datasette.home.bstjohn.net | Deployed (nightly redacted SQLite export of the Postgres DBs) |
...
| Cycling | cycling.home.bstjohn.net | Deployed (static, no auth, browser-side storage) |
| growth-engine | growth-engine.home.bstjohn.net | Deployed (shared Postgres, ForwardAuth infra, migrations via initContainer) |
...
| Forgejo | git.home.bstjohn.net | Deployed (15.0.7, self-built from our patch-and-build repo — see [forgejo.md](forgejo.md); no GitHub push mirrors for migrated repos (#1279), Actions enabled, in-cluster runner; backups: [forgejo-backups.md](forgejo-backups.md)) |
...
| Config Backup (players) | — | CronJob `config-backup-players` on `k3s` (nightly 02:05 Europe/London, same script, covers `plex-config`, `jellyfin-config`, `jellyseerr-config`, `seerr-config`) |
...
| Renovate (Forgejo) | — | CronJob `renovate` (weekly Mon 07:00, 1800s deadline) — dependency PRs on git.home.bstjohn.net, see [forgejo.md](forgejo.md#dependency-updates-renovate) |
