...
#
# To add a new key to authentik-secrets: append it to KEYS below. Do NOT add a
# new per-key migration script.
#
# This script is marked `# migration: repeatable`: run-migrations.sh re-runs it
# on every invocation instead of skipping it once `migration-state` records
# `completed:`. That marker is load-bearing — without it, appending a key here
# is a silent no-op on an existing cluster (state is keyed by filename, not
# content), which is how #963 happened. Every key is guarded by an
# "already exists" check, so re-running costs one `kubectl get` per key.
...
KEYS=(
  ...
  GARDEN_OIDC_CLIENT_SECRET
  EXT_OUTPOST_TOKEN  # API token key for the ext-proxy-outpost (#1111), not OIDC
)
