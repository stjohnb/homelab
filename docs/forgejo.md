...
**Reference.** Read this when working on the Forgejo Git forge itself: storage layout, upgrades, or Actions OIDC. For backups, see [forgejo-backups.md](forgejo-backups.md); for GitHub mirroring, see [forgejo-mirroring.md](forgejo-mirroring.md).
...
**Not currently usable for AWS (or any public cloud IAM OIDC federation):** the issuer URL resolves publicly to a LAN IP (`git.home.bstjohn.net` → `192.168.0.251`), so a cloud provider's IAM can't fetch `/.well-known/openid-configuration` or the JWKS to verify tokens. A split-DNS workaround (a second, publicly-resolvable hostname just for the issuer) was considered and rejected for perudo's AWS deploy — `ROOT_URL` is load-bearing for Authentik SSO callbacks and Forgejo's own clone URLs, so it can't be repointed just to satisfy OIDC discovery. perudo#212 instead uses static, scoped IAM access keys stored as Forgejo Actions secrets, consistent with this repo's static-credential-over-dynamic-auth preference (see [ghcr-auth.md](ghcr-auth.md)). An upstream Forgejo feature request to decouple the Actions OIDC issuer from `ROOT_URL` (codeberg forgejo#14130, filed 2026-08-27) is the gate for revisiting this — until it lands, treat this section as describing the mechanism, not a currently viable deploy path.
