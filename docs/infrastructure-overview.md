...
**Reference.** Read this when touching Traefik, cert-manager, Route53 DNS-01, Flux bootstrap, RBAC, or the CI pipeline. For application-layer services, see [apps-overview.md](apps-overview.md).
...
- **`scripts`** — just `gh`, for jobs that only talk to the GitHub API: `cleanup-actions-storage.yml`, the tag-computation step of `build-arpwatch.yml`, `update-bin-scraper.yml`, and the comment-posting step of `ci.yml`'s `kustomize-diff` job. Kept separate so these don't pay for the scanner toolchain closure.
...
A separate workflow (`.github/workflows/build-arpwatch.yml`) builds the custom arpwatch container image on pushes to `main` (path-filtered to `images/arpwatch/**`) and manual dispatch. It uses the same concurrency pattern (`build-arpwatch-${{ github.ref }}`, `cancel-in-progress: true`) and pushes to `registry.home.bstjohn.net/st-john-software/arpwatch` with a calver tag (`vYYYY-MM-DD.N`, computed from the registry's own tags API) and `latest`. The `build` job sets `DOCKER_BUILD_RECORD_UPLOAD: false` to prevent `docker/build-push-action@v6` from auto-uploading `.dockerbuild` build-record artifacts, which accumulate against the org-shared 2 GB GitHub Actions storage quota. The image build/push, SBOM attestation, and provenance (all pushed to the LAN registry, not uploaded as Actions artifacts) are unaffected.

A second image-build workflow, `.github/workflows/build-forgejo-runner-nix.yml`, builds the Forgejo Actions job image from `images/forgejo-runner-nix/**` with the same concurrency and `DOCKER_BUILD_RECORD_UPLOAD: false` pattern.
...
| `migration-secret-rbac` | `yq` + `scripts/check-migration-secret-rbac.sh` | Verify every Secret a `migrations/*.sh` script reads with `kubectl get secret` has a matching `resourceNames` + `get` grant in `migrations/rbac.yaml` | Yes |
...
Tailscale split DNS for `home.bstjohn.net` points at `100.78.7.18`, served by
...
*Custom…* → `100.78.7.18` → enable *Restrict to search domain* → `home.bstjohn.net`. Record
these exact settings; nothing in this repo can assert them.
