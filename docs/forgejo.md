...
Forgejo's own web sessions are stored in the Forgejo Postgres database (`FORGEJO__session__PROVIDER=db`) with `SESSION_LIFE_TIME=2592000` (30 days), so sessions survive pod restarts. But the session cookie itself is a browser-session cookie (no `Max-Age`), so it is dropped the moment the browser quits — the db provider alone does not keep a user signed in across a browser restart. Patch `0002` (see [Self-built image](#self-built-image)) makes Authentik (OAuth2) logins also issue Forgejo's long-term `persistent` cookie (`LOGIN_REMEMBER_DAYS=30`, tokens recorded in the `forgejo_auth_token` table), so quitting and reopening the browser no longer lands the user back on Forgejo's login page. Signing out of Forgejo clears that cookie.

Diagnostic: many recent signed-in rows in the `session` table with few rows in `forgejo_auth_token` means logins are not getting the long-term cookie.
...
There are two patches. `[actions] OIDC_ISSUER_URL` (`patches/0001-…`) lets the Actions ID-token
...
Upstream declined it as too niche (codeberg forgejo#14130). `patches/0002-…` (#1334) makes
`handleOAuth2SignIn` call `ctx.SetLTACookie(u)` on sign-in, since upstream never issues the
long-term "remember me" cookie for OAuth2 (Authentik) logins the way it does for password logins
with "Remember this device" ticked — see [Authentik SSO](#authentik-sso).
...
- **Version bumps are ordinary PRs** in the build repo: edit `UPSTREAM_VERSION`, CI reports
  ...
  → dispatches fleet-infra's `update-forgejo.yml`, which opens the bump PR here labelled
  `dependencies,auto-bump` — Claws merges it unattended once CI is green and its review is clean,
  same as the other `update-*.yml` receivers (see the Upgrade Runbook below). Renovate on
  `UPSTREAM_VERSION` against Codeberg releases is the longer-term source of that bump PR (#1168).
...
- **Fleet-infra side:** `.github/workflows/update-forgejo.yml` receives the dispatch, validates
  the tag, refuses to open a PR that would move Forgejo to an upstream-version downgrade or to a
  tag already superseded on `main` (the "Refuse downgrade" step), and opens the
  `automation/bump-forgejo-<tag>` PR bumping both `apps/forgejo/deployment.yaml` and
  `apps/forgejo/cronjob-backup.yaml`.
...
`clusters/my-cluster/claws-staging/statefulset.yaml` consumes them as `CLAWS_FORGEJO_TOKEN` and
...
**LGTM does not work as PR approval on any Forgejo-hosted repo** (cycling, bin-scraper, perudo,
forgejo, arpwatch) — `clawsstjohn` is the implicit credential for every Forgejo session Claws
runs, so an `LGTM` comment posted there is the bot approving its own PR and is ignored by the
auto-merger, the same reasoning as the `claws-admin` case above but hitting every PR, not just
admin-capability ones (verified 2026-09-14 on cycling#18: sat with clean review and green CI
until the **Automerge** label was added, then merged within a tick). Add the **Automerge** label
to approve a Claws PR on a Forgejo-hosted repo instead of commenting LGTM. GitHub repos are
unaffected — `gh` there authenticates as the human `stjohnb` account, so `gh pr comment
--body LGTM` still works as approval.
...
1. Open a PR in `St-John-Software/forgejo` bumping `UPSTREAM_VERSION` (review the release's breaking-changes notes). CI runs `prepare.sh`; green means the patch series still applies, red means regenerate the patch (build-repo README, "Changing the patches"). Read the release notes on this PR — it still has a human reviewer. If they warn of a long migration, land a fleet-infra PR widening `startupProbe.failureThreshold` (currently 60 × 10s) **before** merging this PR, and for a risky upgrade also run Stage 1 on demand first (see "Manual runs" in [docs/forgejo-backups.md](forgejo-backups.md)). Merge to `main` — `build-image.yml` builds, pushes `registry.home.bstjohn.net/st-john-software/forgejo:<version>-<sha>-rootless`, and dispatches fleet-infra's `update-forgejo.yml`. Confirm the tag with `GET https://registry.home.bstjohn.net/v2/st-john-software/forgejo/tags/list` using the `ci` credential.
2. `update-forgejo.yml` opens the `automation/bump-forgejo-<tag>` PR here, updating **both** `apps/forgejo/deployment.yaml` **and** `apps/forgejo/cronjob-backup.yaml` — they must always match, because `forgejo dump` reads the database with the schema its own binary expects, so a stale backup image fails outright once the server has migrated (a 14.0.3 dump against a 15.0.5 DB fails with `no such column: remote_address`). `task validate`'s `check-image-consistency` step fails on a mismatch. This PR carries `auto-bump`, and Claws merges it unattended when CI is green and its review is clean. The workflow's "Refuse downgrade" step guards against two mistakes: dispatching an older upstream version than `main` already runs, and re-dispatching a tag that was deployed before and has since been replaced. It leaves one gap: a re-dispatched *older build of the same upstream version* — e.g. `15.0.7-43831f8-rootless` against `15.0.7-dc847b3-rootless` in #1346 — is not detected and would merge unattended, because the guard has no way to order two builds of the same upstream version from tag and git history alone. Such a dispatch only happens when someone re-runs an old `build-image.yml` by hand, so check the dispatched tag against `main` first, or remove `auto-bump` from the resulting PR to force a human merge.
3. This PR merges unattended, so its safety rests on the backups already in place rather than a fresh manual dump: the newest nightly Stage 1 `forgejo-backup` dump (02:30 Europe/London, 14 kept, same disk as `forgejo-data`) is the only backup that covers repositories, and the nightly `postgres-db-backup` `.pgdump` covers the database. Restoring after a bad migration loses writes made since 02:30, unless a manual Stage 1 run (step 1) came first.
4. Flux reconciles in ~1 min once the bump PR merges; `Recreate` terminates the old pod before starting the new one, so expect a brief outage.
...
**Rollback:** Reverting the deployment PR is only safe *before* the new version has migrated the database. Once the pod comes up healthy on the new version and serves traffic, a downgrade requires restoring the newest Stage 1 dump from before the merge onto a fresh PVC — Forgejo does not support backward DB migrations.
...
OIDC is not enabled globally — it must be opted into per workflow or per job with `enable-openid-connect: true`. That key is Forgejo-specific and does the actual work; the standard Actions `permissions: id-token: write` block alone grants nothing here — a job with only `id-token: write` gets no `ACTIONS_ID_TOKEN_REQUEST_*` env vars and any token-fetch step fails (learned during the perudo cutover, see "Consumer setup" below). When `enable-openid-connect: true` is set, the runner injects `ACTIONS_ID_TOKEN_REQUEST_URL` and `ACTIONS_ID_TOKEN_REQUEST_TOKEN` into the job environment, and a token is fetched with:
