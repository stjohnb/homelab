...
**Reference.** Read this when working on the NAS worker node, NFS mounts, or NAS-offline behaviour. For the Plex/Jellyfin exception, see [nas-app-tier-migration.md](nas-app-tier-migration.md).
...
- **The hardlink requirement is the hard part.** Sonarr/Radarr import from Transmission's downloads via `link(2)`, which requires the same **vfsmount**, not merely the same filesystem — `subPath` alone is not sufficient, since each `subPath` mount is its own bind mount and `link(2)` fails across vfsmounts regardless of `st_dev` (#1072). Any SMB layout that must hardlink needs one mount of the share with **no** `subPath` (not `volumeAttributes.subDir`, which would create a second CIFS superblock) plus symlinks for the app-facing paths — and `link(2)`-over-CIFS must be verified against a real mount before any cutover: a silent `EXDEV` fallback to copying cost ~86 GiB of duplication the last time this class of assumption was wrong (nixos-config#51).
...
- **The SMB credential is a plain, imperatively-created Kubernetes Secret — not SOPS-encrypted, not ESO-managed.** Unlike the GHCR PAT (see [ghcr-auth.md](ghcr-auth.md)), it's trivially regenerable at will on the NAS side via `smbpasswd`, so it doesn't need the extra secret-management machinery. Don't propose SOPS/ESO for this credential; that's a deliberate choice, not an oversight (#816).
...
| `k3s-nas` (NixOS 26.05, the NAS host itself) | worker | 192.168.0.128 (static, pinned in NixOS config) | Sonarr, Radarr, Bazarr, Transmission, Immich (all 3 containers), immich-db-backup, config-backup, forgejo-runner-nas |
...
### GitHub Actions runner (native, not k8s)

`k3s-nas` also runs this repo's self-hosted GitHub Actions runner as a native (non-Kubernetes) process — separate from, and not to be confused with, the in-cluster `forgejo-runner-nas` pod (see [forgejo-actions.md](forgejo-actions.md)), which serves Forgejo Actions, not GitHub Actions. This is operational state declared in the sibling `nixos-config` repo, not in this repo's manifests. See [gpu-k3s.md](gpu-k3s.md#github-actions-runner-native-not-k8s) for the fleet-wide picture (LAN-only, `k3s` hosts none, former Hetzner runner decommissioned 2026-08-27).
...
|---------|------------|------------|
| Sonarr | ssdpool-pvc (whole pool at /data, no subPath) | sonarr-local-config-pvc (5Gi) |
| Radarr | ssdpool-pvc (whole pool at /data, no subPath) | radarr-local-config-pvc (5Gi) |
| Bazarr | media-pvc (TV + Movies) | bazarr-local-config-pvc (5Gi) |
| Transmission | ssdpool-pvc (downloads) | transmission-local-config-pvc (5Gi) |
| Immich (server, ML, valkey) | media-pvc (photos) | immich-ml-cache-pvc (10Gi) |
...
### PVC consolidation onto the media export: deferred, not rejected (#1071)

Four NAS-backed PVs exist today: `media-nfs-pv`/`media-pvc`, `media-soft-nfs-pv`/`media-soft-pvc`, and `forgejo-backup-nfs-pv` all point at the same export path (`192.168.0.128:/mnt/SSD-POOL/media`), while `ssdpool-nfs-pv`/`ssdpool-pvc` (used by Sonarr/Radarr/Transmission per #727/#1072) points at the pool root (`/mnt/SSD-POOL`). Consolidating the three media-export PVs onto the pool-root export was considered and **deliberately deferred**, not rejected: `nixos-config#19` (open) will eventually move `media` into its own ZFS dataset (`Array/media`, mounted at `/mnt/SSD-POOL/media`, with `downloads/` moving inside it). Once that lands, NFS can no longer reach `media/` from the pool-root export (no `crossmnt`/`nohide` — that would also re-expose `SSD-POOL/.system` and `SSD-POOL/iocage`), so *every* pool-root consumer, including Sonarr/Radarr, would have to move to the media export anyway. Doing the pool-root consolidation now would mean migrating twice in opposite directions.

**Constraints for whoever eventually does this migration:**
- `nfs.path` is immutable on a bound PV — this is new PV+PVC+pod restarts, never an in-place edit, and Flux must be suspended around the cutover since NFSv3 mounts go stale (`ESTALE`) mid-swap.
- The soft/hard split must survive: readers stay `soft,timeo=50,retrans=2`, writers stay `hard` — at least two new PVs, not one.
- Any hardlinking consumer must mount its claim exactly once with no `subPath` (the vfsmount/`EXDEV` rule from #1072, see [servarr.md](servarr.md#nfs-storage-layout)). None of the three current media-export consumers hardlink today — this constraint only binds Sonarr/Radarr's eventual move onto the media export.
