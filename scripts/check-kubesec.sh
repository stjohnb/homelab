#!/usr/bin/env bash
# Security-scan every workload manifest under ./apps and the claws-staging
# overlays in clusters/my-cluster/ with kubesec and fail on a
...
# The search roots MUST be './'-prefixed (not 'apps'): grep echoes the root
# verbatim, and ALLOWED_EXCEPTIONS above keys on './apps/...' paths. The two
# claws-staging roots keep the StatefulSet and its pre-puller covered after
# they moved out of ./apps into their own Flux Kustomizations (#1373).
mapfile -t WORKLOAD_FILES < <(
  grep -rlE '^kind: (Deployment|StatefulSet|DaemonSet|CronJob|Job)' \
    ./apps ./clusters/my-cluster/claws-staging ./clusters/my-cluster/claws-staging-prepull \
    --include='*.yaml' | sort
)
EXPECTED=${#WORKLOAD_FILES[@]}
echo "Discovered $EXPECTED workload files under ./apps and the claws-staging overlays"
