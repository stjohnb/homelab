  ...
  outputs = { self, nixpkgs }:
    ...
    in
    {
      devShells = forAllSystems (pkgs: {
        ...
        # only talk to remote APIs: cleanup-actions-storage.yml, the bump
        # steps of update-bin-scraper.yml, and the comment-posting step of ci.yml's
        # kustomize-diff job. The tag-computation step of build-arpwatch.yml
        # also lives here, talking to the LAN registry's zot API rather than
        # GitHub. Separate from `default` so those jobs don't pay for the
        # scanner toolchain closure.
        scripts = pkgs.mkShell {
          packages = with pkgs; [
            gh
            curl # build-arpwatch.yml tag step (zot tags API)
            jq # build-arpwatch.yml tag step
