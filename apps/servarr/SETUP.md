...
- Sonarr/Radarr must mount `ssdpool-pvc` exactly once at `/data` with no `subPath`;
  `/tv`, `/movies` and `/downloads` are symlinks into it. Two `subPath` mounts of the
  same PVC are two vfsmounts and `link(2)` still returns `EXDEV` even though `st_dev`
  matches (#1072).
- Check hardlinking directly:
  `kubectl exec deployment/sonarr -c sonarr -- sh -c 'echo t > /downloads/.t && ln /downloads/.t /tv/.t && stat -c %h /tv/.t; rm -f /downloads/.t /tv/.t'` —
  must print `2`.
...
Sonarr's and Radarr's `/tv`/`/movies` and `/downloads` are symlinks into their single `/data`
mount of `ssdpool-pvc` (see below) — the container paths themselves do not move.
...
- Sonarr/Radarr `/tv`, `/movies`, `/downloads` — symlinks into the single `/data` mount of
  `ssdpool-pvc` (whole `/mnt/SSD-POOL`, no `subPath`)
- Transmission `/downloads` — NFS `192.168.0.128:/mnt/SSD-POOL` (`ssdpool-pvc`), `subPath: downloads`

Same `st_dev` is not sufficient for hardlinking: `link(2)` requires the same vfsmount, and each
`volumeMounts` entry is its own bind mount, so two `subPath` mounts of one PVC are still two
vfsmounts and still `EXDEV` (#1072). Sonarr and Radarr mount `ssdpool-pvc` once, with no `subPath`,
and reach their library/downloads paths through symlinks so hardlinking actually works.
