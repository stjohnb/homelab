...
**Depth:** **Reference**
**Read this when:** working on Immich, its pod architecture, photo storage, or its database backups.
**Read instead:** [postgres.md](postgres.md) for shared Postgres details.

Immich is a self-hosted photo/video management platform. It runs as a multi-container pod with three containers sharing localhost networking; the database is the shared `postgres` instance in `apps/postgres/`.
...
└──────────────────────────────────────────────────┘
                      │
                      ▼
          shared `postgres` Service (default)
          see docs/postgres.md
```

All in-pod containers communicate via `localhost` since they share the pod network. `immich-server` reaches Postgres over the cluster network at the `postgres` Service.
...
**Environment**:
- `DB_HOSTNAME=postgres`, `DB_DATABASE_NAME=immich`, `DB_USERNAME=immich`
...
Immich uses the shared instance in [`apps/postgres/`](postgres.md) — `DB_HOSTNAME: postgres` on `immich-server`, database `immich`, role `immich`, password from `immich-db-secret`/`password`.

| Setting | Value |
|---------|-------|
| Host | `postgres` (Service in `default`, see [postgres.md](postgres.md)) |
| Port | 5432 |
| Database / role | `immich` / `immich` |
| Storage | Shared `postgres-data` PVC (local-path) |

The dedicated in-pod `postgres` container (`tensorchord/pgvecto-rs:pg16-v0.2.0`) and the `immich-postgres` Service were removed after migration `0026-immich-db.sh` copied the database into the shared instance, which runs VectorChord for the same CLIP-embedding vector search `pgvecto.rs` provided. `pvc.yaml`'s `immich-postgres-pvc` was retained for one PR as an unmounted rollback copy and has since been deleted.

**NetworkPolicy**: `apps/postgres/networkpolicy.yaml` restricts ingress on port 5432 to pods labeled `app: authentik-server`, `app: authentik-worker`, `app: immich`, `app: garden` or `app: gatus`, plus the backup CronJob pods. A shared-instance restart now disrupts Immich alongside Authentik and Garden.
...
The database itself now lives on the shared `postgres-data` PVC in `apps/postgres/` — see [postgres.md](postgres.md).
...
| Retention | GFS: all dumps from the last 2 days, daily for 14 days, weekly for 8 weeks, monthly for 6 months (~31 files, ~775 MB) — shared script in the `db-backup-retention-script` ConfigMap |
...
**GFS retention (issue #1008).** The flat "keep 14 most recent" policy gave only ~3.5 days of history at the 6-hour schedule — every dump taken before the 2026-04-09/10 loss was rotated away long before the empty-database bug was noticed on 2026-07-28, leaving nothing to restore from. Issue #1008 replaced it with the grandfather-father-son scheme in the table above, shared with `postgres-db-backup` via the `db-backup-retention-script` ConfigMap (see [docs/postgres.md](postgres.md#backups)).

**Orphan cleanup (issue #781).** The re-import wrote a second copy of the library rather than
adopting the old files, so the pre-recovery owner's trees stayed on NFS as pure duplicates:
`upload/409b90bd-2559-4d4f-ace2-57de6f137703` (34 GB), `thumbs/409b90bd-…` (7.7 GB) and
`encoded-video/409b90bd-…` (5.4 GB) — ~47 GB on a share that had reached 93% full. There is no
`profile/409b90bd-…`; its absence is normal. Removing them is operational file deletion on the
NAS, deliberately not a manifest change — nothing in this repo references those paths as
configuration. Verified safe on 2026-08-26 by four checks, all of which must pass before any
`rm -rf`:

1. `select count(*) from asset` ≥ 29118 (read 29165).
2. Zero rows reference the old tree — `select count(*) from asset where "originalPath" like
   '%409b90bd%'`, the same query against `coalesce("encodedVideoPath",'')`, and
   `select count(*) from asset_file where path like '%409b90bd%'` all returned 0.
3. `select id, email from "user"` returns exactly one user, `2ed498d7-02f4-456f-b8db-57ab77ea9cc9`.
4. A dump newer than the recovery exists in `/mnt/SSD-POOL/media/backups/immich/` at **> 17 MB**
   (anything at or below the 16,676,452-byte geodata-only ceiling holds no user data — see Backup
   Validation above). Latest was 179.9 MB with `asset=29138 user=1`.

The live library is `2ed498d7-…`. Confirm the string `409b90bd-2559-4d4f-ace2-57de6f137703` in
every path before deleting; removing the wrong tree destroys the recovered library. The deletion
runs against the mounted photos share, e.g. from the Immich pod:

```bash
kubectl exec -n default deploy/immich -c immich-server -- sh -c '
  set -eu
  U=409b90bd-2559-4d4f-ace2-57de6f137703
  cd /usr/src/app/upload
  for d in upload thumbs encoded-video profile; do
    if [ -d "$d/$U" ]; then echo "removing $d/$U"; rm -rf "./$d/$U"; else echo "$d/$U absent — ok"; fi
  done
  df -h /usr/src/app/upload
'
```

Freed space does not show up immediately: sanoid snapshots pin the `SSD-POOL` root dataset (same mechanism used for the servarr media share — see [servarr.md](servarr.md)), so `df -h` won't reflect a deletion of this size until the snapshots holding those blocks rotate out, potentially weeks later if a recent monthly snapshot is the long pole. Don't treat a flat `df -h` right after deletion as a sign the `rm -rf` failed or re-attempt it.
...
Since #996 the database is the shared `postgres` instance, not an in-pod container — restore targets that Service instead, and a restore now also affects Authentik and Garden. See [postgres.md](postgres.md) for the shared instance's own restore guidance.
...
pg_restore --host=postgres --username=immich --dbname=immich \
  --clean --if-exists /media/backups/immich/immich-db-YYYYMMDD-HHMMSS.pgdump
```

The `--clean --if-exists` flags drop and recreate database objects, making this a full restore of the `immich` database on the shared instance — it does not touch `authentik` or `garden`. Scale Immich back up after restoring: `kubectl scale deployment immich --replicas=1`.

## Deployment Notes

- **Strategy: Recreate** — only one pod at a time (database can't be shared)
- **terminationGracePeriodSeconds: 90** — headroom for graceful shutdown of the pod's containers; the WAL flush, checkpoint, and vector index serialization it originally covered now happen on the shared `postgres` instance instead (see [postgres.md](postgres.md)).
...
- **Startup probes**: All three containers have startup probes to prevent premature liveness/readiness probe kills during slow starts (migrations, ML model loading, and — on `immich-server`'s first start against the shared instance — the VectorChord reindex). Liveness and readiness probes only activate after the startup probe succeeds.
- **Readiness probes**: All three containers have readiness probes. Since all containers share one pod, the pod is only marked Ready when all containers pass readiness — correct behavior since the server can't serve properly if Postgres or ML isn't ready.
