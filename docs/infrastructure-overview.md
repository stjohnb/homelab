...
A fifth workflow (`.github/workflows/update-claws-staging.yml`) handles automated image bumps for `claws-staging`, dispatched by the claws repo's Release workflow (`gh workflow run update-claws-staging.yml --repo St-John-Software/fleet-infra --ref main --field tag=<tag>`, filed as St-John-Software/claws#2759 and required to land after this workflow exists on `main`). It validates the `vYYYY-MM-DD.N` tag format (`^v[0-9]{4}-[0-9]{2}-[0-9]{2}\.[0-9]+$`), rewrites `ghcr.io/st-john-software/claws:*` across `apps/claws/*.yaml` (rather than a hard-coded filename, since claws#2752's StatefulSet rename would break a hard-coded path) and hard-fails if no file in that directory references the image, and opens or updates a single PR on the fixed branch `automation/bump-claws-staging`, labelled `dependencies,auto-bump`. The branch is fixed rather than per-tag because claws cuts a release on every push to `main` — per-tag branches would leave a stack of open PRs based on the same `main`, all but the last of which would conflict on merge. Tracking latest also makes the recurring GHCR tag-pruning breakage (#477, #504, #545) moot, since a pinned stale tag is never left behind long enough to be pruned.

A sixth workflow (`.github/workflows/update-forgejo.yml`) handles automated image bumps for Forgejo, dispatched by `St-John-Software/forgejo`'s `build-image.yml` on `git.home.bstjohn.net` with the org-level `FLEET_INFRA_VERSION_BUMP` PAT. It validates the tag against `^[0-9]+\.[0-9]+\.[0-9]+-[0-9a-f]{7,12}-rootless$`, bumps both `apps/forgejo/deployment.yaml` and `apps/forgejo/cronjob-backup.yaml` atomically — a `grep` guard before and after each `sed` on both files — and opens or updates a PR on a per-tag branch (`automation/bump-forgejo-<tag>`), labelled `dependencies` only. Unlike the other `update-*.yml` receivers, this PR does not carry `auto-bump`: `Recreate` runs a one-way SQLite migration, so the bump needs a manual pre-merge checklist (backup, release notes, possible `startupProbe` widening) rather than merging unattended the moment `ci.yml` passes — see the Upgrade Runbook in [forgejo.md](forgejo.md).
